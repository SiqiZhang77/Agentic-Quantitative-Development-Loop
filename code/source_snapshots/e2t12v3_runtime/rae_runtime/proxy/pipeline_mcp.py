...
def _normalise_tool_output(raw) -> dict | None:
    """Return one MCP result object from the SDK's version-dependent output."""
    if isinstance(raw, dict):
        if raw.get("status") is not None:
            return dict(raw)
        for key in ("structuredContent", "structured_content", "text", "content"):
            if key in raw:
                parsed = _normalise_tool_output(raw.get(key))
    ...
    if isinstance(raw, str):
        try:
            ...
            return None
        return _normalise_tool_output(parsed)
    if isinstance(raw, (list, tuple)):
        for item in raw:
            parsed = _normalise_tool_output(item)
            if parsed is not None:
                return parsed
        return None
    ...
    if callable(model_dump):
        try:
            return _normalise_tool_output(model_dump(mode="json"))
        except (TypeError, ValueError):
            return None
    return None


def _extract_commit_diagnostics(
    result,
    branch_name: str,
    *,
    ...
    - ToolCallOutputItem.output is intentionally typed as Any by the SDK. MCP
      versions have returned a text block, a JSON string, and structured data.
    - The MCP server audit is the authoritative content-free record that a
      backend tool really ran. It may repair an unparsed SDK wrapper, but an
      unaudited SDK item can never invent a success or failure.
    """
    ...
    unverified_write_calls: list[dict] = []

    audit_by_tool: dict[str, list[dict]] = {}
    for record in tool_audit or []:
        if not isinstance(record, dict):
            continue
        tool_name = record.get("tool")
        if isinstance(tool_name, str) and tool_name:
            audit_by_tool.setdefault(tool_name, []).append(record)
    audit_positions: dict[str, int] = {}
    ...
    for item in getattr(result, "new_items", None) or []:
        ...
        output = _normalise_tool_output(raw)
        audit_position = audit_positions.get(tool_name, 0)
        matching_audit = audit_by_tool.get(tool_name) or []
        audited = (
            matching_audit[audit_position]
            if audit_position < len(matching_audit)
            else None
        )
        audit_positions[tool_name] = audit_position + 1
        if output is None and audited is not None:
            output = dict(audited)
        if output is None:
            output = {}
            parse_errors.append(
                {
                    "tool": tool_name,
                    "call_id": call_id,
                    "error": "unparsed SDK output without a matching backend audit record",
                    "raw_output_type": type(raw).__name__,
                }
            )

        status = str(output.get("status") or "unverified")
    ...
    if commit_failures:
        ...
        commit_action = "committed"
        commit_changed = True
    elif commit_action not in {"committed", "skipped"} and unverified_write_calls:
        commit_action = "unknown"
        commit_changed = False
        commit_message = unverified_write_calls[-1]["message"]
    ...
    diagnostics = {
        ...
        "commit": {
            ...
            "unverified_calls": unverified_write_calls,
        },
...
def _attach_post_model_pipeline_evidence(
    exc: BaseException,
    *,
    usage: dict,
    retry_safe: dict,
    ...
    """Keep completed model/commit evidence when later bookkeeping fails."""
    committed_paths = list((retry_safe.get("commit") or {}).get("paths") or [])
    exc.pipeline_out = {
        "status": "failed",
        "usage": dict(usage),
        "artifacts": {
        ...
        "diagnostics": {"retry_safe": retry_safe},
    }
    return exc
...
async def _run_pipeline_mcp_async(
    ...
    retry_safe = _extract_commit_diagnostics(
        result,
        branch_name,
        tool_audit=_read_tool_audit(audit_path),
    )
    ...
    successful_tools = [
        call.get("tool")
        for call in retry_safe.get("tool_calls") or []
        if call.get("status") in {"success", "created", "reused"}
    ]
    if source_path and source_path != target_path:
        if "list_files" not in successful_tools or successful_tools.count("read_file") < 2:
            validation_issues.append(
                "The source and target paths differ, but the run did not demonstrate "
                "broader repository discovery with list_files and multiple read_file calls."
            )
    elif (
        not source_path
        and committed_artifacts
        and not {"read_file", "read_files"}.intersection(successful_tools)
    ):
        # Discovery mode: the file map replaces list_files, so do not demand it —
        # but content committed without reading anything was invented, not derived.
        validation_issues.append(
            "The run committed content without reading any repository file first, "
            "so the change was not grounded in repository evidence."
        )
