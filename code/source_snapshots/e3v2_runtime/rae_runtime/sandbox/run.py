    ...
    *,
    ...
    try:
        ...
        if "architecture_mode" in payload:
            ...
            factory = exp3_boundary_factory or boundary_from_environment
            if not callable(factory):
                raise TypeError("exp3_boundary_factory must be callable")
            exp3_provider_boundary = factory(payload)
            if not manager_star_mode:
                # M0 still enters legacy routing, so prove isolation here before
                # direct-backtest/read-only/pipeline routing can create a branch
                # or call a provider. M1 passes this same boundary to its router,
                # which starts it inside the sanitized pre-orchestration phase.
                exp3_provider_boundary.start(payload)

        if manager_star_mode:
            # M1 receives the same environment-built isolation/provider boundary
            # as M0, then emits directly without entering the legacy
            # iteration/reviewer path or adding a second implementation pass.
            from exp3.router import run_manager_star_runtime
            from exp3.telemetry import build_manager_star_telemetry

            manager_star_result = run_manager_star_runtime(
                payload,
                tracer=tracer,
                production_boundary=exp3_provider_boundary,
            ...
            developer = manager_star_result.get("developer_result") or {}
            implementation_paths = sorted(
                {
                    item.get("path")
                    for item in developer.get("implementation") or []
            ...
            pipeline_out = {
                ...
                "summary": str(final.get("summary") or "Manager-star run completed."),
                "artifacts": {
                    "feature_branch": target_branch,
                    "modified_files": implementation_paths,
                    "new_files": [],
                },
            }
            manager_evaluation = {
        ...
        if exp3_provider_boundary is not None:
            from exp3.telemetry import build_single_agent_telemetry
            ...
            experiment_telemetry = build_single_agent_telemetry(
                ...
                failure_phase=(
                    None if outcome == "success" else "quality_validation"
                ),
                model_alias=exp3_provider_boundary.provider_config.model_alias,
                provider_identity=exp3_provider_boundary.provider_config.identity,
                identity_sha256=exp3_provider_boundary.identity_sha256,
        ...
        experiment_telemetry = None
        if type(e).__name__ == "ManagerStarRunError":
            try:
                from exp3.telemetry import build_manager_star_telemetry
