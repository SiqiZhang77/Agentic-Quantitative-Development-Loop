#!/usr/bin/env python3
"""Preflight all six E2 T3 observations or execute one frozen C0/C1 pair."""
...
OUTPUT_ROOT = (
    ...
    / "02_EXPERIMENT_CONTROL"
    / "status-and-logs"
    / "exp2-terra-t3-rag-observations"
)
AUTHORIZATION_ENV = "E2_T3_RAG_PAIR_AUTHORIZATION"
...
BUILDER = _load_module("exp2_terra_t3_rag_builder", HERE / "build_control_package.py")
...
def load_control(
    ...
    *,
    ...
    for record in runs:
        ...
        if BUILDER.sha256_bytes(
            BUILDER.canonical_bytes(manifest.get("deny_refs"))
        ) != record.get("deny_ref_set_sha256"):
            raise FormalLaunchError("negative-ref set hash mismatch")
        if identity.get("condition") != condition or identity.get("replicate") != replicate:
            raise FormalLaunchError("private condition and run identity differ")
        ...
        if (
            manifest.get("current_target_ref") != record["target_ref"]
            or BUILDER.SOURCE_REF in (manifest.get("deny_refs") or [])
            or record["target_ref"] in (manifest.get("deny_refs") or [])
            or "main" not in (manifest.get("deny_refs") or [])
        ):
...
def _execute_one(
    ...
    *,
    ...
    if not complete_observation:
        raise FormalLaunchError(
            f"incomplete or treatment-invalid observation: {run['submission_id']}"
        )
    return summary
    ...
    *,
    ...
    audit = next(
        (
            path
            for path in (
                artifact_dir / "mcp_tool_audit.jsonl",
                artifact_dir / "exp3_mcp_tool_audit.jsonl",
            )
            if path.is_file()
        ),
        None,
...
def execute_pair(state: dict[str, Any], replicate: int) -> dict[str, Any]:
    ...
    for run in selected:
        ...
        print(
            json.dumps(
                {
                    "schema_version": "exp2-terra-t3-rag-pair-progress-v1",
    ...
    summary = {
        "schema_version": "exp2-terra-t3-rag-pair-result-v1",
        "status": (
            "completed"
