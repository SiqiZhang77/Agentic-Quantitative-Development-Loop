...
def _attempt_retrieval_delivery(
    ...
    """Return one verified, text-free receipt shared by every model-using attempt.
    ...
    back explicitly instead of looking for it on the untouched original request.  A
    later attempt may reuse already-saved answers without making another provider
    call.  Such a zero-call attempt neither received a generator prompt nor needs a
    second delivery receipt; it must not erase valid evidence from an earlier call.
    """
    ...
    if (
        not deliveries
        or len(deliveries) != len(attempt_records)
        or any(not isinstance(item, Mapping) for item in attempt_records)
    ):
        raise RetrievalDeliveryEvidenceError(
            "RAG attempt-delivery evidence is not aligned with attempt accounting"
        )
    model_call_deliveries = []
    for delivery, record in zip(deliveries, attempt_records, strict=True):
        call_count = record.get("provider_call_count")
        if type(call_count) is not int or call_count < 0:
            raise RetrievalDeliveryEvidenceError(
                "RAG attempt accounting has an invalid provider-call count"
            )
        if call_count == 0:
            continue
        if not isinstance(delivery, Mapping):
            raise RetrievalDeliveryEvidenceError(
                "RAG-enabled model call did not preserve generator prompt-delivery evidence"
            )
        model_call_deliveries.append(delivery)
    if not model_call_deliveries:
        raise RetrievalDeliveryEvidenceError(
            "RAG-enabled observation has no model call with prompt-delivery evidence"
        )
    canonical = [
        json.dumps(item, sort_keys=True, separators=(",", ":"))
        for item in model_call_deliveries
    ]
    if len(set(canonical)) != 1:
        raise RetrievalDeliveryEvidenceError(
            "RAG prompt-delivery evidence changed between model-using attempts"
        )
    return copy.deepcopy(dict(model_call_deliveries[0]))
    ...
    *,
    ...
    try:
        ...
        if manager_star_mode:
            ...
            # Each attempt owns a fresh 20-call provider boundary with the
            # per-attempt token cap derived from the frozen observation request. An
