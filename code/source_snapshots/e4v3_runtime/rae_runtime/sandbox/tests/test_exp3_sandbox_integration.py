...
class FakeProductionRoleRunner:
    ...
    def __call__(self, *, role, phase, context, boundary, mcp_launch_spec):
        ...
        call_count = 17 if self.exhaust_developer and role == "developer" else 1
...
def test_m1_uses_released_architect_calls_then_preserves_manager_final_reservation(
    monkeypatch,
    tmp_path,
):
    ...
    # Architect used one planning call, so two of its three reserved calls were
    # transferred to Developer.  The seventeenth Developer call is rejected at
    # the transferred 16-call ceiling, leaving one Manager final-review call
    # reserved within the unchanged 20-call M1 total.
    assert telemetry["role_usage"]["architect"]["calls"] == 1
    assert telemetry["role_usage"]["architect"]["configured_max_calls"] == 3
    assert telemetry["role_usage"]["architect"]["transferred_calls"] == -2
    assert telemetry["role_usage"]["architect"]["max_calls"] == 1
    assert telemetry["role_usage"]["developer"]["calls"] == 16
    assert telemetry["role_usage"]["developer"]["configured_max_calls"] == 14
    assert telemetry["role_usage"]["developer"]["transferred_calls"] == 2
    assert telemetry["role_usage"]["developer"]["max_calls"] == 16
    assert telemetry["role_usage"]["developer"]["call_exhausted"] is True
    assert telemetry["role_usage"]["developer"]["over_budget"] is False
    assert telemetry["shared_budget"]["calls"] == 19
    _assert_response_schema(response)
