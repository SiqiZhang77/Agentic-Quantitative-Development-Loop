...
ROLE_LIMITS: Final = MappingProxyType(
    {
        # These are call reservations for the fixed M1 topology, not three
        # independent resource grants.  The Architect reservation may be
        # released to the Developer only after the Architect has completed its
        # validated planning stage.  The final Manager reservation is never
        # released because M1 must still perform its final review.
...
class RoleBudgetLedger:
    ...
    def __init__(
        ...
        *,
        ...
        # The effective call ceilings begin as the fixed reservations above.
        # A completed Architect stage may transfer only its unused reservation
        # to Developer; the total therefore stays equal to shared_max_calls.
        self._effective_call_limits = {
            role: normalized_limits[role]["max_calls"] for role in sorted(ROLES)
        }
        self._architect_reservation_released = False
    ...
    @classmethod
    def for_manager_star(
        cls, *, shared_max_tokens: int = SHARED_MAX_TOKENS
    ) -> "RoleBudgetLedger":
        """Create the M1 ledger for one request-declared shared token cap."""

        role_limits = {
            role: {
                "max_calls": ROLE_LIMITS[role]["max_calls"],
    ...
    def for_single_agent(
        ...
        M0 and M1 receive the same request-declared 20-call token pool. Manager
        and architect have zero capacity in this ledger because M0 has no such
        stages.
        """

        role_limits = {
            MANAGER: {"max_calls": 0, "max_tokens": 0},
            ARCHITECT: {"max_calls": 0, "max_tokens": 0},
            DEVELOPER: {
                "max_calls": SHARED_MAX_CALLS,
                "max_tokens": shared_max_tokens,
            },
        }
        return cls(
    ...
    def release_unused_architect_calls_to_developer(self) -> int:
        """Transfer unused Architect planning calls to Developer once.

        This method is deliberately narrower than a generic role transfer.  In
        the fixed M1 sequence the Architect stage is the only completed stage
        before Developer begins.  Manager still needs its final review call, so
        releasing a Manager reservation would let Developer consume capacity
        required to finish the manager-star workflow.
        """

        with self._lock:
            if self._architect_reservation_released:
                raise BudgetValidationError(
                    "architect call reservation was already released"
                )
            architect_usage = self._roles[ARCHITECT]
            developer_usage = self._roles[DEVELOPER]
            if architect_usage["pending_calls"]:
                raise BudgetValidationError(
                    "cannot release architect calls while an architect call is pending"
                )
            if developer_usage["calls"]:
                raise BudgetValidationError(
                    "architect calls must be released before developer begins"
                )
            architect_limit = self._effective_call_limits[ARCHITECT]
            unused_calls = architect_limit - architect_usage["calls"]
            if unused_calls < 0:
                raise BudgetValidationError("architect usage exceeds its reservation")

            self._effective_call_limits[ARCHITECT] = architect_usage["calls"]
            self._effective_call_limits[DEVELOPER] += unused_calls
            self._architect_reservation_released = True
            return unused_calls

    def remaining_call_capacity(self, role: object) -> int:
        """Return the number of further model calls this role may start now."""
        ...
        with self._lock:
            role_remaining = self._effective_call_limits[known_role] - self._roles[
                known_role
            ]["calls"]
            shared_remaining = self._shared_max_calls - self._shared_totals_locked()[
                "calls"
            ]
            return max(0, min(role_remaining, shared_remaining))
    ...
    def begin(self, role: object, *, input_hash: object = None) -> str:
        ...
        with self._lock:
            ...
            effective_call_limit = self._effective_call_limits[known_role]
            shared = self._shared_totals_locked()
            if role_usage["calls"] >= effective_call_limit:
                raise BudgetExceeded(
                    f"{known_role} call budget ({effective_call_limit}) exhausted",
                    role=known_role,
    ...
    def _snapshot_locked(self) -> dict:
        ...
        for role, usage in roles.items():
            configured_max_calls = self._role_limits[role]["max_calls"]
            effective_max_calls = self._effective_call_limits[role]
            usage["configured_max_calls"] = configured_max_calls
            usage["transferred_calls"] = effective_max_calls - configured_max_calls
            usage["max_calls"] = effective_max_calls
        ...
        shared.update(
            {
                "configured_max_calls": self._shared_max_calls,
                "transferred_calls": 0,
