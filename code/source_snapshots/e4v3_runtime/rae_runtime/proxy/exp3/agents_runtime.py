...
from exp3.contracts import canonical_json, handoff_character_limit
...
from quant_calculator_audit import read_quant_calculate_records
from research_transcript import capture_model_visible_output
...
async def _close_litellm_async_clients() -> None:
    """Close LiteLLM's cached HTTP clients before their event loop is closed."""

    try:
        from litellm import close_litellm_async_clients
    except ImportError as exc:
        raise AgentsRuntimeUnavailable(
            "LiteLLM client cleanup is unavailable"
        ) from exc
    if not callable(close_litellm_async_clients):
        raise AgentsRuntimeUnavailable("LiteLLM client cleanup is incompatible")
...
def _role_instructions(role: str, phase: str, *, rag_enabled: bool) -> str:
    if role == MANAGER:
        scope = (
    ...
    retrieval_instruction = (
        "Use only the frozen retrieved-memory block already supplied inside the "
        "task context. Do not perform live retrieval, search for other memories, "
        "or request a different RAG result."
        if rag_enabled
        else "Do not invoke retrieval or RAG."
    )
    schema_filename = _HANDOFF_SCHEMA_FILES.get(phase)
    handoff_limit = (
        handoff_character_limit(schema_filename.removesuffix(".schema.json"))
        if schema_filename is not None
        else None
    )
    size_instruction = (
        " The canonical JSON form of this handoff must contain no more than "
        f"{handoff_limit} characters."
        if handoff_limit is not None
        else ""
    )
    return (
        f"Experiment 3 fixed role: {role}. Fixed phase: {phase}. {mediation} "
        f"{scope} {retrieval_instruction} Return exactly one JSON object "
        "matching the supplied output_contract; do not use Markdown fences."
        f"{size_instruction}{final_mapping_instruction}"
    )
...
class AgentsSdkRoleRunner:
    """Execute every fixed topology stage on one explicitly closed event loop."""
    ...
    def __init__(
        ...
        *,
        ...
        litellm_cleanup: Callable[[], Awaitable[None]] | None = None,
    ) -> None:
        ...
        if litellm_cleanup is not None and not callable(litellm_cleanup):
            raise TypeError("litellm_cleanup must be callable or None")
        self._components_loader = components_loader
        self._model_factory = model_factory
        self._litellm_cleanup = litellm_cleanup or _close_litellm_async_clients
        self._event_loop = asyncio.new_event_loop()
        self._requires_litellm_cleanup = False
        self._owned_async_clients: list[object] = []
        self._closed = False
    ...
    def __call__(
        ...
        *,
        ...
        if self._closed:
            raise AgentsRuntimeError("role runner is already closed")
        if boundary.provider_config.provider_mode == COMPANY_LITELLM:
            self._requires_litellm_cleanup = True
    ...
    def close(self) -> None:
        """Wait for cached HTTP clients, then close the shared event loop once."""

        if self._closed:
            return
        self._closed = True
        try:
            if self._owned_async_clients:
                self._event_loop.run_until_complete(
                    self._close_owned_async_clients()
                )
            if self._requires_litellm_cleanup:
                self._event_loop.run_until_complete(self._litellm_cleanup())
        finally:
            ...
            self._event_loop.run_until_complete(
                self._event_loop.shutdown_default_executor()
            )
            self._event_loop.close()

    async def _close_owned_async_clients(self) -> None:
        """Await each direct OpenAI HTTP client created for a role stage."""

        seen: set[int] = set()
        for client in self._owned_async_clients:
            ...
            if not callable(close):
                raise AgentsRuntimeUnavailable(
                    "owned OpenAI client does not expose async cleanup"
                )
            result = close()
            if hasattr(result, "__await__"):
                await result
        self._owned_async_clients.clear()
    ...
    async def _run_stage(
        ...
        *,
        ...
        max_turns = (
            boundary.ledger.remaining_call_capacity(known_role)
            if phase == "developer_to_manager"
            else _PHASE_MAX_TURNS[phase]
        )
        if max_turns <= 0:
            raise AgentsRuntimeError("role phase has no remaining model-call capacity")
        components = self._components_loader()
        raw_model = self._model(known_role, components, boundary)
        owned_client = getattr(raw_model, "_rae_owned_async_client", None)
        if owned_client is not None:
            self._owned_async_clients.append(owned_client)
        ...
        schema_filename = _HANDOFF_SCHEMA_FILES.get(phase)
        if schema_filename is not None:
            handoff_limit = handoff_character_limit(
                schema_filename.removesuffix(".schema.json")
            )
            if handoff_limit is not None:
                prompt_value["output_constraints"] = {
                    "canonical_json_character_limit": handoff_limit,
                }
        if phase == "manager_final":
        ...
        rag_enabled = (
            ...
            and type(task.get("retrieved_memory_context")) is str
            and bool(task.get("retrieved_memory_context"))
        )
        ...
        async with components.MCPServerStdio(
            ...
            agent = components.Agent(
                ...
                instructions=_role_instructions(
                    known_role, phase, rag_enabled=rag_enabled
                ),
                model=model,
        ...
        task = context.get("task") if isinstance(context, Mapping) else None
        observation_attempt = (
            task.get("observation_attempt") if isinstance(task, Mapping) else None
        )
        attempt = (
            observation_attempt.get("number")
            if isinstance(observation_attempt, Mapping)
            else None
        )
        capture_model_visible_output(
            mcp_launch_spec.env["MCP_TOOL_AUDIT_PATH"],
        ...
        # Save exactly what the model made visible before enforcing the JSON
        # handoff contract.  A malformed answer remains available for restricted
        # qualitative analysis even though it cannot drive the next role.
        return _parse_final_output(raw_output)
...
class ProductionManagerStarRoleAdapters:
    ...
    def close(self) -> None:
        """Close the shared model-network runtime when the five stages finish."""

        closer = getattr(self._runner, "close", None)
        if callable(closer):
            closer()

    def _validated_audit_records(self) -> list[dict[str, Any]]:
        ...
        for item in records[1:]:
            ...
            if (
                role == DEVELOPER
                and tool
                in {"commit_and_push", "replace_in_file", "commit_calculation_artifact"}
                and item.get("status") == "success"
            ):
                path = item.get("path")
                if type(path) is not str or not path.strip():
                    raise McpAuditError(
                        "successful developer write omitted its committed path"
                    )
                if tool == "commit_calculation_artifact":
                    artifact_sha256 = item.get("artifact_sha256")
                    artifact_bytes = item.get("artifact_bytes")
                    reference_count = item.get("calculation_reference_count")
                    if (
                        ...
                        raise McpAuditError(
                            "successful calculation artifact omitted its content hash"
                        )
                    if type(artifact_bytes) is not int or not 1 <= artifact_bytes <= 1_048_576:
                        raise McpAuditError(
                            "successful calculation artifact byte count is invalid"
                        )
                    if type(reference_count) is not int or not 1 <= reference_count <= 512:
                        raise McpAuditError(
                            "successful calculation artifact reference count is invalid"
                        )
        return records

    def commit_count(self) -> int:
        records = self._validated_audit_records()
        return sum(
    ...
    def committed_paths(self) -> list[str]:
        """Return only paths backed by a successful developer write audit record."""
    ...
    def calculation_tool_records(self) -> list[dict[str, Any]]:
        """Return strictly sanitized quant_calculate evidence from the shared audit."""

        # Reuse the complete M1 audit validation first so a malformed sibling
        # record cannot be hidden by filtering only calculator calls.
        self._validated_audit_records()
        return read_quant_calculate_records(self._audit_path)
...
def build_production_role_adapters(
    ...
    *,
    ...
    from exp3.attempt_control import audit_filename

    audit_path = artifact_dir / audit_filename(payload, "exp3_mcp_tool_audit.jsonl")
    audit_header = {
        "schema_version": _MCP_AUDIT_SCHEMA_VERSION,
        "record_type": "audit_initialized",
        "architecture_mode": "manager_star",
    ...
    active_runner = runner or AgentsSdkRoleRunner()
    try:
        ...
        closer = getattr(active_runner, "close", None)
        if callable(closer):
            try:
                closer()
            except Exception:
                # Preserve the adapter-construction error that prevented any role
                # or model call; cleanup must not replace that primary evidence.
                pass
        raise
