...
ARCHITECT_PLAN_MAX_CANONICAL_CHARS = 50_000
DEVELOPER_RESULT_MAX_CANONICAL_CHARS = 50_000
...
_HANDOFF_SPECS = {
    "architect_plan_v1": _HandoffSpec(
        ...
        producer="architect",
        char_limit=ARCHITECT_PLAN_MAX_CANONICAL_CHARS,
    ),
    "developer_result_v1": _HandoffSpec(
        filename="developer_result_v1.schema.json",
        producer="developer",
        char_limit=DEVELOPER_RESULT_MAX_CANONICAL_CHARS,
    ),
...
def handoff_character_limit(expected_schema: str) -> int | None:
    """Return the frozen canonical-character limit for one handoff schema."""

    spec = _HANDOFF_SPECS.get(expected_schema)
    if spec is None:
        raise HandoffValidationError(expected_schema, ["unsupported handoff version"])
    return spec.char_limit
