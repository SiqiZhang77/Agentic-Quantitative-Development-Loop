...
def boundary_from_environment(
    ...
    *,
    ...
    attempt_token_cap = None
    if current_attempt is not None:
        validate_two_attempt_controls(payload)
        attempt_token_cap = token_budget_per_attempt(payload)
    if continuation_attempt:
        context = payload.get("_experiment_attempt") or {}
        if context.get("prior_status") not in CONTINUATION_PRIOR_STATUSES:
            raise ProductionProviderBoundaryUnavailable(
    ...
    token_cap = attempt_token_cap or SHARED_MAX_TOKENS
    ledger = (
        RoleBudgetLedger.for_single_agent(shared_max_tokens=token_cap)
        if mode == SINGLE_AGENT
        else RoleBudgetLedger.for_manager_star(shared_max_tokens=token_cap)
    )
