...
def test_real_m1_adapter_runs_fixed_topology_through_accounted_models(
    ...
    # The fake Architect used one of its three reserved calls.  Its two unused
    # planning calls transfer to Developer; Manager's final-review call remains
    # reserved, so the complete five-stage workflow still totals at most 20.
    assert [turns for _phase, turns, _prompt in FakeRunner.phases] == [1, 3, 1, 16, 1]
    architect_prompt = next(
        prompt
        for phase, _turns, prompt in FakeRunner.phases
        if phase == "architect_to_manager"
    )
    ...
    assert architect_prompt["output_constraints"] == {
        "canonical_json_character_limit": 50_000,
    }
    assert developer_prompt["output_constraints"] == {
        "canonical_json_character_limit": 50_000,
    }
    ...
    for index, agent in enumerate(FakeAgent.created):
        if index in manager_indexes:
            ...
            assert "no more than 50000 characters" in agent.instructions
