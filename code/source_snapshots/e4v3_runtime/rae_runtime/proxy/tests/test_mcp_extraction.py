...
def test_turn_limit_diagnostics_treat_calculation_artifact_as_validation_and_commit():
    error = AgentTurnLimitExceeded(
        turn_limit=12,
        turns_consumed=12,
        audit=[
            {
                ...
                "path": "experiments/shared/submissions/result.json",
            }
        ],
    )

    retry_safe = error.pipeline_out["diagnostics"]["retry_safe"]
    assert retry_safe["validation_reached"] is True
    assert retry_safe["commit_reached"] is True
...
def test_extract_commit_diagnostics_preserves_batch_read_paths():
    result = SimpleNamespace(
        final_output="done",
        new_items=[
            *_fake_call(
                ...
                {
                    "status": "success",
                    "paths": ["public/input.csv", "src/helper.py"],
                },
            )
        ],
    )
...
@pytest.mark.parametrize(
    "read_call",
    [
        {"tool": "read_file", "status": "success", "path": "public/input.csv"},
        {
            "tool": "read_files",
            "status": "success",
            "paths": ["public/input.csv"],
        },
        {
            "tool": "read_files",
            "status": "success",
            "paths": ["src/helper.py", "public/input.csv"],
        },
    ],
)
def test_discovery_guard_accepts_successful_read_of_declared_source(read_call):
    assert _repository_discovery_issues(
        source_path="public/input.csv",
        target_path="submissions/result.md",
        committed_artifacts=[{"path": "submissions/result.md"}],
        tool_calls=[
...
def test_discovery_guard_rejects_missing_or_unproven_source_read(tool_calls):
    assert _repository_discovery_issues(
        source_path="public/input.csv",
        target_path="submissions/result.md",
        committed_artifacts=[{"path": "submissions/result.md"}],
        ...
        "repository discovery with list_files and a successful read of the "
...
def test_extract_commit_diagnostics_accepts_calculation_artifact_commit():
    result = SimpleNamespace(
        final_output="done",
        new_items=[
            *_fake_call(
                ...
                "commit_calculation_artifact",
                {
                    "status": "success",
                    "repo_name": "bankingscience/BSLAgenticQuantDevLoop",
                    "branch": "quant/E3V2T3Q5-T3-R1-M0",
                    "path": "experiments/shared/submissions/result.json",
    ...
    diag = _extract_commit_diagnostics(result, "quant/E3V2T3Q5-T3-R1-M0")

    assert diag["commit"]["action"] == "committed"
    assert diag["commit"]["paths"] == ["experiments/shared/submissions/result.json"]
    assert _missing_commit_call(diag) is False
