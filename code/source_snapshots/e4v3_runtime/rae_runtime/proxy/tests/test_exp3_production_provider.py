...
def test_exp2_single_agent_boundary_preserves_true_rag_state() -> None:
    payload = _payload()
    ...
    assert boundary.snapshot()["rag_enabled"] is True
    mismatched = _boundary(payload, rag_enabled=False)
    with pytest.raises(ProductionProviderBoundaryError, match="RAG state"):
        mismatched.start(payload)


def test_factorial_manager_star_boundary_preserves_true_rag_state() -> None:
    ...
    assert boundary.snapshot()["architecture_mode"] == "manager_star"
    assert boundary.snapshot()["rag_enabled"] is True
...
def test_m0_receives_20_calls_from_the_shared_stable_budget():
    payload = _payload()
    boundary = _boundary(payload)
    boundary.start(payload)
    ...
    assert boundary.ledger.snapshot()["shared"]["calls"] == 20
    # One pre-orchestration record plus exactly one pre-model check per admitted
    # call. The rejected 21st attempt is stopped by the capacity precheck first.
    assert len(boundary.preflight.evidence()) == 21
...
def test_environment_loader_requires_frozen_hashes_and_uses_injected_probe(tmp_path):
    ...
    assert boundary.ledger.snapshot()["roles"]["developer"]["max_calls"] == 20
...
def test_attempt_two_boundary_accepts_every_registered_continuation_status(
    ...
    payload["_experiment_attempt"] = {
        "number": 2,
        "maximum": 2,
        "prior_status": prior_status,
        "feedback": "No answer correctness information is available.",
    }
    value = _manifest_value(payload)
    manifest = NegativeRefManifest.from_dict(value)
    path = tmp_path / "negative-ref.json"
    ...
    boundary = boundary_from_environment(
        payload,
        environment=environment,
        ref_exists=lambda _repo, ref: ref in existing,
    )
    evidence = boundary.start(payload)

    assert evidence["passed"] is True
    assert evidence["current_target_absence_checked"] is False


def test_attempt_two_boundary_rejects_unregistered_continuation_status(tmp_path):
    payload = _payload()
    payload["iteration_controls"] = {
        "allow_iteration": True,
        "max_iterations": 2,
    ...
    payload["_experiment_attempt"] = {
        "number": 2,
        "maximum": 2,
        "prior_status": "unknown_continuation_reason",
        "feedback": "No answer correctness information is available.",
    }
    value = _manifest_value(payload)
    manifest = NegativeRefManifest.from_dict(value)
    ...
    with pytest.raises(
        ProductionProviderBoundaryUnavailable,
        match="valid prior attempt status",
    ):
        boundary_from_environment(
            payload,
            environment=environment,
...
def test_two_attempt_boundary_uses_request_declared_500k_attempt_cap(tmp_path):
    payload = _payload()
    payload["iteration_controls"] = {
        "allow_iteration": True,
        "max_iterations": 2,
    ...
    payload["_experiment_attempt"] = {"number": 1, "maximum": 2}
    value = _manifest_value(payload)
    manifest = NegativeRefManifest.from_dict(value)
    path = tmp_path / "negative-ref.json"
    path.write_text(json.dumps(value), encoding="utf-8")
    ...
    assert boundary.ledger.snapshot()["shared"]["max_tokens"] == 500_000
