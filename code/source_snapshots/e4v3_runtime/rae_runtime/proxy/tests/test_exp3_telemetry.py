...
def test_builds_versioned_content_free_manager_star_telemetry() -> None:
    ...
    assert telemetry["schema_version"] == "exp3-runtime-telemetry-v4"
...
def test_two_attempt_manager_accounting_accepts_different_transfer_outcomes() -> None:
    """A role's effective capacity may differ while the frozen policy stays fixed."""

    values = []
    for transferred in (2, 1):
        outcome = _outcome()
        budget = outcome["budget"]
        budget["shared"].update(
            configured_max_calls=20,
            transferred_calls=0,
            max_calls=20,
            max_tokens=500_000,
        )
        budget["roles"]["manager"].update(
            configured_max_calls=3,
            transferred_calls=0,
            max_calls=3,
            max_tokens=500_000,
        )
        budget["roles"]["architect"].update(
            configured_max_calls=3,
            transferred_calls=-transferred,
            max_calls=3 - transferred,
            max_tokens=500_000,
        )
        budget["roles"]["developer"].update(
        ...
        values.append(
            {
                ...
                "provider_calls": copy.deepcopy(outcome["provider_calls"]),
                "isolation_preflight": copy.deepcopy(outcome["isolation_preflight"]),
    ...
    aggregate = aggregate_attempt_accountings(
        values, architecture_mode="manager_star", planned_attempts=2
    )

    assert aggregate["budget"]["shared"]["configured_max_calls"] == 40
    assert aggregate["budget"]["shared"]["transferred_calls"] == 0
    assert aggregate["budget"]["shared"]["max_calls"] == 40
    assert aggregate["budget"]["roles"]["architect"]["configured_max_calls"] == 6
    assert aggregate["budget"]["roles"]["architect"]["transferred_calls"] == -3
    assert aggregate["budget"]["roles"]["architect"]["max_calls"] == 3
    assert aggregate["budget"]["roles"]["developer"]["configured_max_calls"] == 28
    assert aggregate["budget"]["roles"]["developer"]["transferred_calls"] == 3
    assert aggregate["budget"]["roles"]["developer"]["max_calls"] == 31


def test_two_attempt_manager_accounting_still_rejects_fixed_budget_drift() -> None:
    first = _single_agent_snapshot()
    second = copy.deepcopy(first)
    second["budget"]["shared"]["configured_max_calls"] = 16
    second["budget"]["shared"]["max_calls"] = 16

    with pytest.raises(TelemetryError, match="budget identity changed"):
        aggregate_attempt_accountings(
            [first, second], architecture_mode="single_agent", planned_attempts=2
        )


def test_attempt_accounting_rejects_inconsistent_effective_transfer_math() -> None:
    value = _single_agent_snapshot()
    value["budget"]["roles"]["developer"]["transferred_calls"] = 1

    with pytest.raises(TelemetryError, match="configured capacity plus transfer"):
        aggregate_attempt_accountings(
            [value], architecture_mode="single_agent", planned_attempts=1
        )
...
def test_two_attempt_rag_manager_keeps_ten_attempt_scoped_deliveries() -> None:
    ...
    for role, calls in {"manager": 6, "architect": 2, "developer": 2}.items():
        budget["roles"][role].update(
            ...
            configured_max_calls={"manager": 6, "architect": 6, "developer": 28}[role],
            transferred_calls=0,
...
def test_formal_two_attempt_budget_requires_attempt_records_for_both_arms() -> None:
    outcome = _outcome()
    outcome["budget"]["shared"]["configured_max_calls"] = 40
    ...
    snapshot = _single_agent_snapshot()
    snapshot["budget"]["shared"]["configured_max_calls"] = 40
    snapshot["budget"]["shared"]["max_calls"] = 40
    snapshot["budget"]["roles"]["developer"]["configured_max_calls"] = 40
