...
def test_read_and_write_tool_sets_are_exact():
    ...
    assert CALCULATION_TOOLS == frozenset(
        {
            "quant_calculate",
            "submit_t3_items",
    ...
    assert ALL_TOOLS == READ_TOOLS | DEVELOPER_WRITE_TOOLS | CALCULATION_TOOLS
...
@pytest.mark.parametrize("role", [MANAGER, ARCHITECT])
@pytest.mark.parametrize("tool", sorted(CALCULATION_TOOLS))
def test_manager_and_architect_cannot_use_calculation_tool(role, tool):
    assert tool_allowed(role, tool) is False
    with pytest.raises(ToolPermissionError):
        require_tool_permission(role, tool)


def test_developer_may_use_the_request_bound_calculation_tool():
    assert tool_allowed(DEVELOPER, "quant_calculate") is True
    assert require_tool_permission(DEVELOPER, "quant_calculate") is None
    assert tool_allowed(DEVELOPER, "submit_t3_items") is True
    assert require_tool_permission(DEVELOPER, "submit_t3_items") is None
    assert tool_allowed(DEVELOPER, "submit_calculation_checkpoint") is True
    assert require_tool_permission(DEVELOPER, "submit_calculation_checkpoint") is None
    assert tool_allowed(DEVELOPER, "commit_calculation_artifact") is True
    assert require_tool_permission(DEVELOPER, "commit_calculation_artifact") is None
