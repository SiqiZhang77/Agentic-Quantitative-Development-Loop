...
from quant_calculator import quant_calculator_enabled
...
def freeze_retrieval_prompt_delivery(
    payload: dict[str, Any],
) -> tuple[str, dict[str, Any] | None]:
    """Render one frozen evidence block and its matching text-free receipt.

    A manager-star run calls this once before its five role stages.  Reusing the
    returned string prevents a later stage from rendering a different view of
    the same already-frozen gateway retrieval context.  This function formats
    local request data only; it does not execute retrieval.
    """
...
def retrieval_prompt_delivery(payload: dict[str, Any]) -> dict[str, Any] | None:
    ...
    return freeze_retrieval_prompt_delivery(payload)[1]
...
def build_ticket_context(payload: dict[str, Any]) -> str:
    ...
    attempt = payload.get("_experiment_attempt")
    if isinstance(attempt, dict):
        lines.extend(
            [
                (
                    "Prospectively bounded observation attempt: "
                    f"{_safe(attempt.get('number'))} of {_safe(attempt.get('maximum'))}."
                ),
                _safe(attempt.get("feedback")),
            ]
        )
    lines.extend([
        "Description:",
...
def build_mcp_code_prompt(
    ...
    *,
    ...
    calculator_profile = quant_calculator_enabled(payload)
    if target_path and calculator_profile:
        validation_step = (
            "4. Follow the frozen task text exactly. It is the sole source of answer-\n"
            "   capture, validation and completion requirements for this task profile."
        )
        commit_step = (
            "5. Do not add an outer prompt completion rule. Use the available tools only in\n"
            "   the order and for the purposes stated by the frozen task text."
        )
        commit_reminder = (
            "The outer repository workflow does not make a target-file commit a condition for\n"
            "capturing or scoring answers; the shared frozen task governs both architectures."
        )
    elif target_path:
        validation_step = (
