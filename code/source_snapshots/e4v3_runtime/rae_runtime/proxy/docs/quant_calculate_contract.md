...
- Empty-vector and empty-matrix rejections state that the minimum accepted
  length or row count is one and the actual count is zero. They also tell the
  caller that it may correct the named argument and call the tool again in the
  same run. The calculator never invents or fills missing values.
