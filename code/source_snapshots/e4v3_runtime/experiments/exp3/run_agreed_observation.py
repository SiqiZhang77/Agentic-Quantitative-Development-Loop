...
DEFAULT_AGREEMENT_DIR = HERE / "agreement-terra-v2"
DEFAULT_OUTPUT_ROOT = (
    PROJECT_ROOT
...
REMOTE_NAME = "github"
EXPECTED_EXECUTION_EPOCH = "E3V2"
T3_QUANT_EXECUTION_EPOCH = "E3V2T3Q1"
T3_QUANT_TARGET_PREFIX = f"quant/{T3_QUANT_EXECUTION_EPOCH}-"
T3_QUANT_TARGET_PATH = (
    "experiments/shared/t3-quant-suite-v1/submissions/"
    "quant_portfolio_analytics.json"
)
T3_QUANT_READ_SCOPE = ["experiments/shared/t3-quant-suite-v1"]
T3_QUANT_TOOL_PROFILE = {
...
def _agreement_profile(agreement: dict[str, Any]) -> dict[str, Any]:
    """Return one of the two exact E3 agreement profiles; reject all others."""

    schema_version = agreement.get("schema_version")
    if schema_version == "exp3-terra-agreement-v2":
        if agreement.get("execution_epoch") != EXPECTED_EXECUTION_EPOCH:
            raise LaunchError("agreement is not the active E3V2 execution epoch")
        if agreement.get("formal_run_count") != 18:
            raise LaunchError("E3V2 agreement run count is invalid")
        return {
            "kind": "original_e3v2",
            "execution_epoch": EXPECTED_EXECUTION_EPOCH,
            "target_ref_prefix": f"quant/{EXPECTED_EXECUTION_EPOCH}-",
        }
    if schema_version == "exp3-t3-quant-agreement-v1":
        if agreement.get("execution_epoch") != T3_QUANT_EXECUTION_EPOCH:
            raise LaunchError("agreement is not the T3 quantitative amendment epoch")
        if agreement.get("scope") != "prospective_T3_only":
            raise LaunchError("T3 quantitative agreement scope is invalid")
        if agreement.get("formal_run_count") != 6:
            raise LaunchError("T3 quantitative agreement run count is invalid")
        if agreement.get("target_ref_prefix") != T3_QUANT_TARGET_PREFIX:
            raise LaunchError("T3 quantitative target namespace is invalid")
        if agreement.get("tool_profile") != T3_QUANT_TOOL_PROFILE:
            raise LaunchError("T3 quantitative tool profile is invalid")
        runs = agreement.get("runs")
        if not isinstance(runs, list) or len(runs) != 6 or any(
            not isinstance(item, dict) or item.get("task_id") != "T3"
            for item in runs
        ):
            raise LaunchError("T3 quantitative agreement run inventory is invalid")
        return {
            "kind": "t3_quant_v1",
            "execution_epoch": T3_QUANT_EXECUTION_EPOCH,
            "target_ref_prefix": T3_QUANT_TARGET_PREFIX,
        }
    raise LaunchError("agreement schema is not an approved E3 execution profile")


def _image_ref(agreement: dict[str, Any]) -> str:
    digest = agreement["image_digest"]
    reference = f"{IMAGE_REPOSITORY}@{digest}"
    # Docker Desktop can intermittently fail to resolve a locally built
    # repository@digest alias even while the exact immutable image ID remains
    # present.  Inspect the agreed full ID directly and verify equality; keep
    # the repository-qualified form only as the human-facing identity.
    completed = subprocess.run(
    ...
    if completed.stdout.strip() != digest:
        raise LaunchError(
            "agreed Docker image ID mismatch: "
            f"expected {digest}, observed {completed.stdout.strip() or 'empty'}"
        )
    return reference
...
def preflight(agreement_dir: Path, run_id: str | None) -> dict[str, Any]:
    ...
    profile = _agreement_profile(agreement)
    ...
    if not selected.get("target_ref", "").startswith(profile["target_ref_prefix"]):
        raise LaunchError("target ref is outside the active agreement namespace")
    parameters = (
        (request.get("execution_objectives") or {}).get("parsed_task_parameters")
        or {}
    )
    if profile["kind"] == "t3_quant_v1":
        repository_details = request.get("repository_details")
        if not isinstance(repository_details, list) or len(repository_details) != 1:
            raise LaunchError("T3 quantitative request repository scope is invalid")
        objectives = request.get("execution_objectives") or {}
        if parameters.get("quant_calculator_enabled") is not True:
            raise LaunchError("T3 quantitative request did not enable its calculator")
        if objectives.get("target_path") != T3_QUANT_TARGET_PATH:
            raise LaunchError("T3 quantitative request target path is invalid")
        if repository_details[0].get("allowed_directories") != T3_QUANT_READ_SCOPE:
            raise LaunchError("T3 quantitative request read scope is invalid")
        if "retrieval_context" in request:
            raise LaunchError("T3 quantitative request must omit retrieval context")
    elif "quant_calculator_enabled" in parameters:
        raise LaunchError("original E3V2 request cannot enable the T3 calculator")
...
def _docker_run_command(
    *,
    ...
    # Execute by the immutable image ID recorded in the agreement.  This is the
    # same image represented by state["image_ref"], but avoids Docker Desktop's
    # transient local repository@digest alias-resolution failures.
    execution_image = state["agreement"]["image_digest"]
    command.extend(
        [
            "-e",
        ...
        json.dumps(
            {
                ...
                "execution_epoch": agreement["execution_epoch"],
...
def main() -> None:
    ...
    try:
        ...
        safe = {
            "status": "ready",
            "execution_epoch": state["agreement"]["execution_epoch"],
