...
This checked-in version is intentionally unable to create an agreement or run
a model. Source ref/commit, image, runtime schemas, tool fingerprints, public
task hashes and private evaluator hashes remain `__UNRESOLVED_...__`. In
addition, both agreement generation and model execution are disabled by the
configuration schema. A later reviewed version must replace all bindings and
enable only agreement generation first. Model execution must remain disabled
while the agreement is written and checked; a separate later launcher version
