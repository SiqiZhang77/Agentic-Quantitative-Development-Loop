...
# The protocol-level experiment identity remains frozen.  EXECUTION_EPOCH
# distinguishes the repaired implementation/restart without redefining E3.
EXPERIMENT_ID = "E3-manager-star-v1"
EXECUTION_EPOCH = "E3V2"
SUPERSEDES_AGREEMENT_SHA256 = (
    "29251f45b56ddc86a3cd7272c653cc9957d14417b057c2878599887136cec3ee"
)
SOURCE_REF = "exp3/terra-source-v7"
SOURCE_COMMIT = "f6d8e7c65510387426ae7f693ca7eda3f7ddfa46"
IMAGE_DIGEST = "sha256:f45e58f07f143ac5d04600e3b5db95ffa7955f5c28926b35503c0ccd765f3e0a"
...
def _known_negative_refs(
    ...
    prior_e3_targets = sorted(
        ref
        for ref in remote_heads
        if ref.startswith("quant/E3-") or ref.startswith("quant/E3V")
    )
    e2_refs = sorted(
        ref
        for ref in remote_heads
        if ref not in v5_refs
        and ref not in prior_e3_targets
        and (
    ...
    return v5_refs, e2_refs, prior_e3_targets
...
def _target_ref(task_id: str, replicate: int, arm: str) -> str:
    return f"quant/{EXECUTION_EPOCH}-{task_id}-R{replicate}-{arm}"
...
def build(output: Path, *, replace: bool = False) -> dict[str, Any]:
    ...
    v5_refs, e2_refs, preexisting_e3_targets = _known_negative_refs(remote_heads)
    ...
    prior_targets: list[str] = list(preexisting_e3_targets)
    ...
    try:
        ...
        agreement = {
            ...
            "supersedes_agreement_sha256": SUPERSEDES_AGREEMENT_SHA256,
            "prior_agreement_status": "discontinued_incomplete",
            "prior_observations_included_in_v2_estimate": False,
            "restart_reason_codes": [
                "T1_SOURCE_PARENT_DIRECTORY_MISSING",
                "M1_ACCEPTANCE_IDENTITY_PROMPT_INCOMPLETE",
                "SHARED_DISCOVERY_VALIDATOR_MISMATCH",
                "LATE_FAILURE_TELEMETRY_REPAIR",
            ],
            "restart_reason": (
                "Runtime validation, manager-final contract and telemetry repairs "
                "require a clean implementation epoch; v1.5 observations remain "
                "immutable calibration evidence and are excluded from v2 analysis."
            ),
            "status": "user_agreed_preflight_ready",
            "approval_authority": "user",
            "task_policy": "retain_public_T1_T2_T3_and_current_rubric_metrics",
            "formal_run_count": 18,
            "formal_observation_count_at_issue": 0,
        ...
        _write_json(
            ...
            {
                "schema_version": "exp3-terra-agreement-digest-v2",
...
def main() -> None:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=HERE / "agreement-terra-v2",
    )
