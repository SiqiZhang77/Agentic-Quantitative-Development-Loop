#!/usr/bin/env python3
"""Resolve the E3V9 draft from reviewed public and hash-only inputs.
...
E3V9_ROOT = CONTROL_ROOT / "experiments" / "exp3" / "e3v9"
PROSPECTIVE_CONFIG = E3V9_ROOT / "config" / "e3v9.prospective.json"
CONTROL_FILES = (
    "experiments/exp3/e3v9/controls.py",
    "experiments/exp3/e3v9/build_e3v9_agreement.py",
    "experiments/exp3/e3v9/preflight_e3v9.py",
    "experiments/exp3/e3v9/schemas/e3v9_config.schema.json",
    "experiments/prospective_freezes/e3v9/collect_public_bindings.py",
    "experiments/prospective_freezes/e3v9/collect_private_evaluator_bindings.py",
    "experiments/prospective_freezes/e3v9/build_freeze_config.py",
    "experiments/prospective_freezes/e3v9/build_scorer_result_v2.py",
    "experiments/prospective_freezes/e3v9/model_visible_trace_schema_v1.json",
)


class FreezeConfigError(RuntimeError):
    """The supplied evidence cannot safely resolve the E3V9 draft."""
...
def _controls_module():
    path = E3V9_ROOT / "controls.py"
    spec = importlib.util.spec_from_file_location("e3v9_freeze_controls", path)
    if spec is None or spec.loader is None:
        raise FreezeConfigError("cannot load E3V9 controls")
...
def _validate_public(value: dict[str, Any]) -> None:
    if value.get("schema_version") != "shared-t3-e3v9-public-binding-inventory-v1":
        raise FreezeConfigError("public binding inventory schema is not E3V9 v1")
...
def _validate_private(value: dict[str, Any]) -> None:
    ...
    if value["schema_version"] != "exp3-e3v9-private-evaluator-hashes-v1":
        raise FreezeConfigError("private evaluator manifest schema is not E3V9 v1")
