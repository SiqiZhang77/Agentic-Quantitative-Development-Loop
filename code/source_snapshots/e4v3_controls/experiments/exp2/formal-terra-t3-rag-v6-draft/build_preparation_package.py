...
PAIR_ORDER: Final = {1: ("C0", "C1"), 2: ("C1", "C0"), 3: ("C0", "C1")}
...
def execution_schedule() -> list[dict[str, Any]]:
    ...
    if len(schedule) != 6 or len({(x["replicate"], x["condition"]) for x in schedule}) != 6:
        raise AssertionError("schedule must contain three complete C0/C1 pairs")
    return schedule
    ...
    try:
        ...
        manifest = {
            ...
            "next_required_gate": (
                ...
                "a separate prospective agreement and six-observation zero-model preflight"
            ),
        }
