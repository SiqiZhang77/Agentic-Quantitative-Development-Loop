#!/usr/bin/env python3
"""Preflight all ten E2V7 observations or execute one C0/C1 pair."""
...
OUTPUT_ROOT = (
    ...
    / "02_EXPERIMENT_CONTROL"
    / "status-and-logs"
    / "exp2-terra-t3-rag-v7-observations"
)
AUTHORIZATION_ENV = "E2V7_PAIR_AUTHORIZATION"
...
BUILDER = _load_module("exp2_terra_t3_rag_v7_builder", HERE / "build_control_package.py")
PROJECTION = _load_module("exp2_e2v7_item_store_projection", HERE / "item_store_projection.py")
...
def _validate_evaluator_manifest(value: Any) -> dict[str, str]:
    expected = {
        ...
        "evaluator_coordinator_sha256": sha256_file(BUILDER.RESULT_ADAPTER_PATH),
        "visible_response_trace_schema_sha256": sha256_file(BUILDER.TRACE_SCHEMA_PATH),
        "item_store_projection_sha256": sha256_file(BUILDER.ITEM_STORE_PROJECTION_PATH),
    }
    if value != expected:
...
def load_control(
    ...
    *,
    ...
    if (
        ...
        or agreement.get("formal_run_count") != 10
        or agreement.get("pair_count") != 5
    ):
    ...
    if (
        ...
        or agreement.get("private_item_scorer_sha256")
        != BUILDER.PRIVATE_ITEM_SCORER_SHA256
        or agreement.get("private_base_scorer_sha256")
        != BUILDER.PRIVATE_BASE_SCORER_SHA256
        or agreement.get("evaluator_coordinator_sha256")
        != sha256_file(BUILDER.RESULT_ADAPTER_PATH)
        or agreement.get("visible_response_trace_schema_sha256")
        != sha256_file(BUILDER.TRACE_SCHEMA_PATH)
        or agreement.get("item_store_projection_sha256")
        != sha256_file(BUILDER.ITEM_STORE_PROJECTION_PATH)
    ...
    for record in runs:
        ...
        if condition not in {"C0", "C1"} or replicate not in set(range(1, 6)):
        ...
        if (
            ...
            or parameters.get("t3_item_submission_enabled") is not True
            or parameters.get("t3_item_submission_schema_path")
            != str(BUILDER.T3_RELATIVE_ROOT / "item_submission_schema_v2.json")
            or parameters.get("t3_primary_outcome") != "item_correctness_25"
    ...
    if retrieval_hashes != {agreement.get("retrieval_context_sha256")}:
        raise FormalLaunchError("the five C1 requests do not share one frozen retrieval")
...
def _preflight_scorer(agreement: Mapping[str, Any]) -> dict[str, Any]:
    checks = (
        (BUILDER.PRIVATE_ITEM_SCORER_PATH, agreement.get("private_item_scorer_sha256")),
        (BUILDER.PRIVATE_BASE_SCORER_PATH, agreement.get("private_base_scorer_sha256")),
        (BUILDER.RESULT_ADAPTER_PATH, agreement.get("evaluator_coordinator_sha256")),
        (BUILDER.TRACE_SCHEMA_PATH, agreement.get("visible_response_trace_schema_sha256")),
        (BUILDER.ITEM_STORE_PROJECTION_PATH, agreement.get("item_store_projection_sha256")),
    )
    for path, expected in checks:
        if not path.is_file() or sha256_file(path) != expected:
            raise FormalLaunchError("private/public scoring control hash does not match agreement")
...
def preflight(
    ...
    *,
    ...
    expected_count = 10 if replicate is None else 2
    ...
    records = []
    for run in selected:
        record = _preflight_record(run, state["agreement_sha256"])
    ...
    summary = {
        ...
        "pair_count": 5 if replicate is None else 1,
        "conditions": ["C0", "C1"],
    ...
    with tempfile.TemporaryDirectory(prefix="e2-t3-score-git-") as raw:
        ...
        source_fetched = subprocess.run(
            [
                ...
                "https://github.com/bankingscience/BSLAgenticQuantDevLoop.git",
                f"refs/heads/{BUILDER.SOURCE_REF}:refs/heads/score-source",
            ],
            capture_output=True,
            text=True,
        )
        if source_fetched.returncode != 0:
            raise FormalLaunchError("frozen source branch could not be read for scoring")
        source_head = subprocess.run(
            ["git", "-C", str(bare), "rev-parse", "refs/heads/score-source"],
            capture_output=True,
            text=True,
        )
        if source_head.returncode or source_head.stdout.strip() != BUILDER.SOURCE_COMMIT:
            raise FormalLaunchError("remote source branch drifted before scoring")
        ancestry = subprocess.run(
            [
                "git", "-C", str(bare), "merge-base", "--is-ancestor",
                "refs/heads/score-source", "refs/heads/score-target",
            ],
            capture_output=True,
            text=True,
        )
        if ancestry.returncode:
            raise FormalLaunchError("target branch does not contain the frozen source history")
        ...
        if changed_run.returncode:
            raise FormalLaunchError("target branch could not be compared with frozen source")
...
def _merge_attempt_trace(artifact_dir: Path) -> Path | None:
    numbered = [
        artifact_dir / "model_visible_transcript_attempt_1.jsonl",
        artifact_dir / "model_visible_transcript_attempt_2.jsonl",
    ]
    present = [path.is_file() for path in numbered]
    unnumbered = artifact_dir / "model_visible_transcript.jsonl"
    if present == [False, False]:
        return unnumbered if unnumbered.is_file() else None
    if present == [False, True] or unnumbered.exists():
        raise FormalLaunchError("model-visible transcript attempt files are inconsistent")
    rows: list[str] = []
    for path, exists in zip(numbered, present, strict=True):
        if not exists:
            continue
        rows.extend(_validated_jsonl_lines(path))
    destination = artifact_dir / "model_visible_transcript_merged.jsonl"
    if destination.exists():
        raise FormalLaunchError("refusing to overwrite merged model-visible transcript")
...
def _validate_visible_trace(path: Path | None) -> None:
    if path is None:
        return
    schema = _load_json(BUILDER.TRACE_SCHEMA_PATH)
    validator = Draft202012Validator(schema)
    for line_number, raw in enumerate(path.read_text(encoding="utf-8").splitlines(), start=1):
        value = json.loads(raw)
        if list(validator.iter_errors(value)):
            raise FormalLaunchError(
                f"model-visible transcript violates frozen schema at line {line_number}"
            )
...
def _score(
    *,
    ...
    if sha256_file(BUILDER.PRIVATE_ITEM_SCORER_PATH) != agreement.get(
        "private_item_scorer_sha256"
    ):
        raise FormalLaunchError("private T3 item scorer hash does not match agreement")
    if sha256_file(BUILDER.PRIVATE_BASE_SCORER_PATH) != agreement.get(
        "private_base_scorer_sha256"
    ):
        raise FormalLaunchError("private T3 base scorer hash does not match agreement")
    artifact_dir = output_directory / "output" / "artifacts"
    artifact_dir.mkdir(parents=True, exist_ok=True)
    ...
    visible = _merge_attempt_trace(artifact_dir)
    ...
    if item_store.is_file():
        ...
        projection_receipt = PROJECTION.project_item_store(
            item_store, scorer_item_store, projection_receipt_path
        )
    private_score_path = artifact_dir / "t3_item_score_private_v1.json"
    command = [
        sys.executable,
        str(BUILDER.PRIVATE_ITEM_SCORER_PATH),
        "--csv",
        ...
        "--base-scorer",
        str(BUILDER.PRIVATE_BASE_SCORER_PATH),
        "--audit",
        str(audit),
        "--scope-passed",
    ...
    if completed.returncode or not private_score_path.is_file():
        raise FormalLaunchError("private T3 item scoring failed")
    public_score_path = artifact_dir / "t3_item_score_v2.json"
    adapter = [
        sys.executable,
        str(BUILDER.RESULT_ADAPTER_PATH),
        "--private-score",
        str(private_score_path),
        "--validator",
        str(BUILDER.RESULT_VALIDATOR_PATH),
        "--output",
        str(public_score_path),
    ]
    if visible is not None:
        adapter.extend(["--visible-trace", str(visible)])
    adapted = subprocess.run(adapter, capture_output=True, text=True)
    if adapted.returncode or not public_score_path.is_file():
        raise FormalLaunchError("answer-blind v2 result conversion failed")
    score = _load_json(public_score_path)
    return {
        **score,
        "submission_id": run["submission_id"],
        "condition": run["condition"],
        ...
        "score_path": str(public_score_path),
...
def _public_score_summary(score: Mapping[str, Any]) -> dict[str, Any]:
    """Summarize the frozen v2 item result without reading removed v1 fields."""
    required = (
        "correct_items",
        "total_items",
        "submitted_item_ids",
    ...
    missing_fields = [field for field in required if field not in score]
    if missing_fields:
        raise FormalLaunchError(
            "public T3 score is missing required v2 fields: "
            + ", ".join(missing_fields)
        )
    item_results = score["item_results"]
    if not isinstance(item_results, list):
        raise FormalLaunchError("public T3 score item_results must be a list")
    states = (
        "accepted",
        "invalid_format",
        "explicit_abstain",
    ...
    return {
        ...
        "submission_states": {
            state: sum(
                ...
                and row.get("submission_state") == state
...
def execute_pair(state: dict[str, Any], replicate: int) -> dict[str, Any]:
    ...
    for run in selected:
        ...
        try:
            ...
            scores.append(score)
            score_summary = _public_score_summary(score)
        print(
            json.dumps(
                {
...
def main() -> int:
    ...
    parser.add_argument("--replicate", type=int, choices=range(1, 6))
