...
from experiments.exp3.e3v16 import build_e3v16_launch_package as launch
from experiments.exp3.e3v16 import controls
from experiments.exp3.e3v16 import run_e3v16_observation as observation
...
def _identity(arm: str = "M0") -> dict:
    ...
    value = {
        "run_id": f"E3V16-T3-R1-{arm}",
        "replicate": 1,
        "sequence_in_pair": 1 if arm == "M0" else 2,
        "arm": arm,
        "architecture_mode": architecture,
        "target_ref": f"quant/E3V16-T3-R1-{arm}",
        "paired_target_ref": f"quant/E3V16-T3-R1-{paired_arm}",
        "source": {
...
def test_launch_scripts_do_not_import_provider_clients() -> None:
    for name in (
        "build_e3v16_launch_package.py",
        "run_e3v16_observation.py",
        "run_e3v16_pair.py",
    ):
...
def test_execute_creates_item_store_parent_before_container(
    ...
    negative.write_text("{}\n", encoding="utf-8")
    state = {
        "manifest": {"experiment_id": "E3V16"},
        "run_id": "E3V16-T3-R1-M0",
        "agreement_sha256": "a" * 64,
        "launch_package_sha256": "b" * 64,
        "target_ref": "quant/E3V16-T3-R1-M0",
    ...
    monkeypatch.setenv(
        "E3V16_RUN_AUTHORIZATION", "AUTHORIZE_E3V16-T3-R1-M0"
    )
    monkeypatch.setattr(
        observation,
        "_credentials",
