...
from experiments.exp3.e3v9 import build_e3v9_launch_package as launch
from experiments.exp3.e3v9 import controls
from experiments.exp3.e3v9 import run_e3v9_observation as observation
...
def _resolved_config() -> dict:
    ...
    config["source"]["source_ref"] = "exp/shared-t3-runtime-v1"
...
def _identity(arm: str = "M0") -> dict:
    ...
    value = {
        "run_id": f"E3V9-T3-R1-{arm}",
        "replicate": 1,
        "sequence_in_pair": 1 if arm == "M0" else 2,
        "arm": arm,
        "architecture_mode": architecture,
        "target_ref": f"quant/E3V9-T3-R1-{arm}",
        "paired_target_ref": f"quant/E3V9-T3-R1-{paired_arm}",
        "source": {
            "source_ref": "exp/shared-t3-runtime-v1",
            "source_commit": "b" * 40,
        },
    }
...
def test_request_contains_complete_task_and_matches_runtime_schema() -> None:
    ...
    assert request["iteration_controls"]["max_token_budget_per_run"] == 700_000
...
def test_launch_scripts_do_not_import_provider_clients() -> None:
    for name in (
        "build_e3v9_launch_package.py",
        "run_e3v9_observation.py",
        "run_e3v9_pair.py",
    ):
