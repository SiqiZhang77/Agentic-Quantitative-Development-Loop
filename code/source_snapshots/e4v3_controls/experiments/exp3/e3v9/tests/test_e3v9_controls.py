...
def _resolved_bindings_for_agreement() -> dict:
    ...
    replacements = {
        ...
        "__UNRESOLVED_E3V9_CONTROL_REF__": "exp/shared-t3-runtime-freeze-v1",
        "__UNRESOLVED_E3V9_CONTROL_COMMIT_SHA__": "d" * 40,
    }
    ...
    control_path = "experiments/exp3/e3v9/controls.py"
...
def test_resolved_bindings_require_the_frozen_agreement_for_preflight() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        output = Path(tmp) / "agreement"
        agreement = build_e3v9_agreement.build_agreement(config, output)
        assert output.is_dir()
        ready = preflight_e3v9.preflight(
            config,
            replicate=2,
...
def test_launch_stage_keeps_agreement_external_and_requires_explicit_authorization() -> None:
    observation = (controls.HERE / "run_e3v9_observation.py").read_text(
        encoding="utf-8"
    )
    pair = (controls.HERE / "run_e3v9_pair.py").read_text(encoding="utf-8")
