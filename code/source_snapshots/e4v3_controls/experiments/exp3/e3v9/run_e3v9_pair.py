#!/usr/bin/env python3
"""Preflight or serially execute one E3V9 T3 M0/M1 pair."""
...
try:
    from . import run_e3v9_observation as observation
except ImportError:  # pragma: no cover - direct script execution
    import run_e3v9_observation as observation  # type: ignore[no-redef]
...
def _fetch_submission(
    *, source_ref: str, source_commit: str, target_ref: str, destination: Path
    ...
    with tempfile.TemporaryDirectory(prefix="e3v9-score-git-") as raw:
...
def _score(
    ...
    if item_store.is_file():
        command.extend(["--item-store", str(item_store)])
...
def main() -> int:
    ...
    try:
        ...
        print(
            json.dumps(
                {
                    "schema_version": "exp3-e3v9-pair-result-v1",
                    "status": "failed",
