"""Offline-only identity and readiness checks for the E3V9 draft.
...
CONFIG_PATH = HERE / "config" / "e3v9.prospective.json"
SCHEMA_PATH = HERE / "schemas" / "e3v9_config.schema.json"
...
class ConfigError(ValueError):
    """The prospective E3V9 configuration violates a design constraint."""
...
class NotReadyError(RuntimeError):
    ...
    def __init__(self, issues: Iterable[str]):
        ...
        super().__init__("E3V9 is not ready: " + "; ".join(self.issues))
...
def validate_config(config: dict[str, Any]) -> None:
    ...
    if config["conditions"] != EXPECTED_CONDITIONS:
        raise ConfigError("M0 and M1 do not match the E3V9 architecture contrast")
    if config["arm_order"] != EXPECTED_ARM_ORDER:
        raise ConfigError("E3V9 must retain the alternating E3V8 pair order")
    public_paths = {
    ...
    if config["execution"]["formal_model_execution_enabled"]:
        raise ConfigError("the E3V9 preparation file must not enable model execution")
    if (
...
def _run_id(replicate: int, arm: str) -> str:
    return f"E3V9-T3-R{replicate}-{arm}"
...
def build_run_identity(
    ...
    identity = {
        "schema_version": "exp3-e3v9-run-identity-draft-v1",
...
def validate_pair(pair: list[dict[str, Any]]) -> None:
    if len(pair) != 2:
        raise ConfigError("an E3V9 pair must contain exactly two observations")
