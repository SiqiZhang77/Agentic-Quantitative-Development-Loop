#!/usr/bin/env python3
"""Preflight or execute exactly one E3V9 observation."""
...
def _validate_package(package: Path) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if (
        ...
        not in {
            "exp3-e3v9-launch-package-v1",
            "exp3-e3v10-launch-package-v1",
        }
        or manifest.get("experiment_id") not in {"E3V9", "E3V10"}
        ...
        raise ObservationLaunchError("launch package is not an approved E3V9/E3V10 profile")
...
def preflight(package: Path, run_id: str) -> dict[str, Any]:
    ...
    if controls.sha256_value({key: value for key, value in identity.items() if key != "identity_sha256"}) != identity.get("identity_sha256"):
        # E3V9 identities hash the complete object before the identity field is added.
...
def execute(state: dict[str, Any], output_root: Path) -> dict[str, Any]:
    ...
    if result_present:
        try:
            ...
            result_status = value.get("status") if isinstance(value, dict) else None
...
def main() -> int:
    ...
    try:
        ...
        print(
            json.dumps(
                {
                    "schema_version": "exp3-e3v9-observation-launch-error-v1",
                    "status": "failed",
