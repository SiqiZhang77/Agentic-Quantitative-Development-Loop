"""Offline-only identity and readiness checks for the E3V14 draft.
...
CONFIG_PATH = HERE / "config" / "e3v14.prospective.json"
SCHEMA_PATH = HERE / "schemas" / "e3v14_config.schema.json"
...
EXPECTED_CONTROL_REF = "exp/e3v14-missing-item-continuation-controls"
...
REGISTERED_CHANGES_FROM_E3V13 = {
    "reason": "allow_clean_missing_item_attempt_to_continue",
    ...
    "required_items_missing_added_to_provider_continuation_statuses": True,
...
class ConfigError(ValueError):
    """The prospective E3V14 configuration violates a design constraint."""
...
class NotReadyError(RuntimeError):
    ...
    def __init__(self, issues: Iterable[str]):
        ...
        super().__init__("E3V14 is not ready: " + "; ".join(self.issues))
...
def validate_config(config: dict[str, Any]) -> None:
    ...
    if config["conditions"] != EXPECTED_CONDITIONS:
        raise ConfigError("M0 and M1 do not match the E3V14 architecture contrast")
    if config["arm_order"] != EXPECTED_ARM_ORDER:
        raise ConfigError("E3V14 must retain the alternating E3V8 pair order")
    public_paths = {
    ...
    if (
        ...
        raise ConfigError(
            "E3V14 must transfer only unused Architect planning calls after a "
            "validated handoff while reserving Manager's final review call"
        )
    if config["execution"]["formal_model_execution_enabled"]:
        raise ConfigError("the E3V14 preparation file must not enable model execution")
    if (
    ...
    if (
        config["freeze_status"] == "ready_for_freeze"
        and config["source"]["source_ref"] != "exp/shared-t3-runtime-v5"
    ):
        raise ConfigError("E3V14 must bind the published shared runtime v5 source ref")
    if (
        config["freeze_status"] == "ready_for_freeze"
        and config["control"]["control_ref"] != EXPECTED_CONTROL_REF
    ):
        raise ConfigError("E3V14 must bind the dedicated freeze-control ref")
...
def _run_id(replicate: int, arm: str) -> str:
    return f"E3V14-T3-R{replicate}-{arm}"
...
def build_run_identity(
    ...
    identity = {
        "schema_version": "exp3-e3v14-run-identity-draft-v1",
...
def validate_pair(pair: list[dict[str, Any]]) -> None:
    if len(pair) != 2:
        raise ConfigError("an E3V14 pair must contain exactly two observations")
