#!/usr/bin/env python3
"""Preflight or execute exactly one E3V11 observation."""
...
def _validate_package(package: Path) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if (
        manifest.get("schema_version") != "exp3-e3v11-launch-package-v1"
        or manifest.get("experiment_id") != "E3V11"
        ...
        raise ObservationLaunchError("launch package is not the approved E3V11 profile")
...
def preflight(package: Path, run_id: str) -> dict[str, Any]:
    ...
    if controls.sha256_value({key: value for key, value in identity.items() if key != "identity_sha256"}) != identity.get("identity_sha256"):
        # E3V11 identities hash the complete object before the identity field is added.
...
def main() -> int:
    ...
    try:
        ...
        print(
            json.dumps(
                {
                    "schema_version": "exp3-e3v11-observation-launch-error-v1",
                    "status": "failed",
