#!/usr/bin/env python3
"""Preflight or serially execute one E3V17 T3 M0/M1 pair."""
...
try:
    from . import run_e3v17_observation as observation
    from .item_store_projection import project_item_store
except ImportError:  # pragma: no cover - direct script execution
    import run_e3v17_observation as observation  # type: ignore[no-redef]
...
_SAFE_SCORER_ERROR = re.compile(r"^[a-z0-9_]{1,96}$")


def _private_scorer_failure_code(completed: subprocess.CompletedProcess[str]) -> str:
    """Return only a value-free scorer error code, never scorer output or answers."""
    ...
    for stream in (completed.stderr, completed.stdout):
        for line in reversed((stream or "").splitlines()):
            ...
            if isinstance(code, str) and _SAFE_SCORER_ERROR.fullmatch(code):
                return code
    return "unavailable"
...
def _fetch_submission(
    *, source_ref: str, source_commit: str, target_ref: str, destination: Path
    ...
    with tempfile.TemporaryDirectory(prefix="e3v17-score-git-") as raw:
...
def _score(
    ...
    if completed.returncode or not legacy_score.is_file():
        raise PairLaunchError(
            "private item scoring failed; scorer_code="
            + _private_scorer_failure_code(completed)
        )
...
def main() -> int:
    ...
    try:
        ...
        print(
            json.dumps(
                {
                    "schema_version": "exp3-e3v17-pair-result-v1",
                    "status": "failed",
