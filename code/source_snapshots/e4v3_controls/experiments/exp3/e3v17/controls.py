"""Offline-only identity and readiness checks for the E3V17 draft.
...
CONFIG_PATH = HERE / "config" / "e3v17.prospective.json"
SCHEMA_PATH = HERE / "schemas" / "e3v17_config.schema.json"
...
EXPECTED_CONTROL_REF = "exp/e2v7-e3v17-shared-controls"
EXPECTED_CALL_TRANSFER_POLICY = {
    "source_role": "architect",
    "recipient_role": "developer",
    "release_after": "validated_architect_handoff",
    "manager_final_review_call_reserved": True,
}
REGISTERED_CHANGES_FROM_E3V16 = {
    "reason": "continue_item_scoring_after_one_public_schema_invalid_candidate",
    "per_attempt_model_call_cap": {"from": 20, "to": 20},
    "per_attempt_token_cap": {"from": 500000, "to": 500000},
    "per_observation_model_call_cap": {"from": 40, "to": 40},
    "per_observation_token_cap": {"from": 1000000, "to": 1000000},
    "manager_star_topology_changed": False,
    "manager_star_call_transfer_policy_changed": False,
    "telemetry_schema_version": {"from": "v4", "to": "v4"},
    "required_items_missing_added_to_provider_continuation_statuses": False,
    "rag_delivery_receipt_aggregation_changed": False,
    "host_observation_result_status_path": {
        "from": "execution_summary.status",
        "to": "execution_summary.status",
    },
    "host_observation_result_classification_changed": False,
    "private_item_scorer_changed": True,
    "invalid_item_scoring_disposition": {
        "from": "abort_complete_score",
        "to": "mark_that_item_incorrect_and_continue",
    },
    "scorer_failure_code_reporting_changed": True,
...
class ConfigError(ValueError):
    """The prospective E3V17 configuration violates a design constraint."""
...
class NotReadyError(RuntimeError):
    ...
    def __init__(self, issues: Iterable[str]):
        ...
        super().__init__("E3V17 is not ready: " + "; ".join(self.issues))
...
def validate_config(config: dict[str, Any]) -> None:
    ...
    if config["conditions"] != EXPECTED_CONDITIONS:
        raise ConfigError("M0 and M1 do not match the E3V17 architecture contrast")
    if config["arm_order"] != EXPECTED_ARM_ORDER:
        raise ConfigError("E3V17 must retain the alternating E3V8 pair order")
    public_paths = {
    ...
    if (
        ...
        raise ConfigError(
            "E3V17 must transfer only unused Architect planning calls after a "
            "validated handoff while reserving Manager's final review call"
        )
    if config["execution"]["formal_model_execution_enabled"]:
        raise ConfigError("the E3V17 preparation file must not enable model execution")
    if (
    ...
    if (
        config["freeze_status"] == "ready_for_freeze"
        and config["source"]["source_ref"] != "exp/shared-t3-runtime-v6"
    ):
        raise ConfigError("E3V17 must bind the published shared runtime v6 source ref")
    if (
        config["freeze_status"] == "ready_for_freeze"
        and config["control"]["control_ref"] != EXPECTED_CONTROL_REF
    ):
        raise ConfigError("E3V17 must bind the dedicated freeze-control ref")
...
def _run_id(replicate: int, arm: str) -> str:
    return f"E3V17-T3-R{replicate}-{arm}"
...
def build_run_identity(
    ...
    identity = {
        "schema_version": "exp3-e3v17-run-identity-draft-v1",
...
def validate_pair(pair: list[dict[str, Any]]) -> None:
    if len(pair) != 2:
        raise ConfigError("an E3V17 pair must contain exactly two observations")
