...
from experiments.exp3.e3v17 import build_e3v17_agreement, controls, preflight_e3v17


def _resolved_bindings_for_agreement() -> dict:
    config = copy.deepcopy(controls.load_config())
    replacements = {
        "__UNRESOLVED_E3V17_SOURCE_REF__": "exp/shared-t3-runtime-v6",
        "__UNRESOLVED_E3V17_SOURCE_COMMIT_SHA__": "a" * 40,
        "__UNRESOLVED_E3V17_IMAGE_REF__": "exp3-e3v17-runtime",
        "__UNRESOLVED_E3V17_IMAGE_DIGEST__": "sha256:" + "b" * 64,
        "__UNRESOLVED_E3V17_CONTROL_REF__": controls.EXPECTED_CONTROL_REF,
        "__UNRESOLVED_E3V17_CONTROL_COMMIT_SHA__": "d" * 40,
    }
    ...
    control_path = "experiments/exp3/e3v17/controls.py"
...
def test_current_builder_refuses_before_writing_any_agreement() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        ...
        with pytest.raises(controls.NotReadyError):
            build_e3v17_agreement.build_agreement(controls.load_config(), output)
        assert not output.exists()


def test_zero_model_preflight_reports_no_observation_or_external_call() -> None:
    result = preflight_e3v17.preflight(controls.load_config(), replicate=1)
...
def test_registered_change_is_only_item_score_failure_isolation() -> None:
    change = controls.REGISTERED_CHANGES_FROM_E3V16

    assert change["host_observation_result_status_path"] == {
        "from": "execution_summary.status",
        "to": "execution_summary.status",
    }
    assert change["host_observation_result_classification_changed"] is False
    assert change["private_item_scorer_changed"] is True
    assert change["invalid_item_scoring_disposition"] == {
        "from": "abort_complete_score",
        "to": "mark_that_item_incorrect_and_continue",
    }
    assert change["scorer_failure_code_reporting_changed"] is True
...
def test_resolved_public_hash_mismatch_keeps_preflight_unready() -> None:
    ...
    result = preflight_e3v17.preflight(config, replicate=1)
...
def test_ready_config_rejects_a_different_control_branch() -> None:
    config = _resolved_bindings_for_agreement()
    config["control"]["control_ref"] = "exp/e3v17-freeze-controls"
...
def test_resolved_bindings_require_the_frozen_agreement_for_preflight() -> None:
    config = _resolved_bindings_for_agreement()
    result = preflight_e3v17.preflight(config, replicate=2)
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        output = Path(tmp) / "agreement"
        agreement = build_e3v17_agreement.build_agreement(config, output)
        assert output.is_dir()
        ready = preflight_e3v17.preflight(
            config,
            replicate=2,
...
def test_launch_stage_keeps_agreement_external_and_requires_explicit_authorization() -> None:
    observation = (controls.HERE / "run_e3v17_observation.py").read_text(
        encoding="utf-8"
    )
    pair = (controls.HERE / "run_e3v17_pair.py").read_text(encoding="utf-8")
    ...
    assert not (controls.HERE / "agreement-terra-e3v17").exists()
