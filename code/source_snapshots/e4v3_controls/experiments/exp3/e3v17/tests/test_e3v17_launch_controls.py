...
from experiments.exp3.e3v17 import build_e3v17_launch_package as launch
from experiments.exp3.e3v17 import controls
from experiments.exp3.e3v17 import run_e3v17_observation as observation
from experiments.exp3.e3v17 import run_e3v17_pair as pair
...
def _resolved_config() -> dict:
    ...
    config["source"]["source_ref"] = "exp/shared-t3-runtime-v6"
...
def _identity(arm: str = "M0") -> dict:
    ...
    value = {
        "run_id": f"E3V17-T3-R1-{arm}",
        "replicate": 1,
        "sequence_in_pair": 1 if arm == "M0" else 2,
        "arm": arm,
        "architecture_mode": architecture,
        "target_ref": f"quant/E3V17-T3-R1-{arm}",
        "paired_target_ref": f"quant/E3V17-T3-R1-{paired_arm}",
        "source": {
...
def test_launch_scripts_do_not_import_provider_clients() -> None:
    for name in (
        "build_e3v17_launch_package.py",
        "run_e3v17_observation.py",
        "run_e3v17_pair.py",
    ):
...
def test_private_scorer_error_reporting_keeps_only_safe_code() -> None:
    completed = SimpleNamespace(
        stderr=(
            '{"status":"failed","error":"candidate_artifact_schema_invalid"}\n'
            "untrusted diagnostic containing candidate material"
        ),
        stdout="",
    )

    assert pair._private_scorer_failure_code(completed) == (
        "candidate_artifact_schema_invalid"
    )


def test_private_scorer_error_reporting_rejects_free_text() -> None:
    completed = SimpleNamespace(
        stderr='{"error":"candidate value was 0.123"}\n',
        stdout="",
    )

    assert pair._private_scorer_failure_code(completed) == "unavailable"
...
def test_execute_creates_item_store_parent_before_container(
    ...
    negative.write_text("{}\n", encoding="utf-8")
    state = {
        "manifest": {"experiment_id": "E3V17"},
        "run_id": "E3V17-T3-R1-M0",
        "agreement_sha256": "a" * 64,
        "launch_package_sha256": "b" * 64,
        "target_ref": "quant/E3V17-T3-R1-M0",
    ...
    monkeypatch.setenv(
        "E3V17_RUN_AUTHORIZATION", "AUTHORIZE_E3V17-T3-R1-M0"
    )
    monkeypatch.setattr(
        observation,
        "_credentials",
    ...
    def fake_run(*args, **kwargs):
        ...
        (host_output / "result.json").write_text(
            json.dumps({"execution_summary": {"status": "succeeded"}}),
            encoding="utf-8",
        )
