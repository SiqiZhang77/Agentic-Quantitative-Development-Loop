"""Offline-only identity and readiness checks for the E3V13 draft.
...
CONFIG_PATH = HERE / "config" / "e3v13.prospective.json"
SCHEMA_PATH = HERE / "schemas" / "e3v13_config.schema.json"
...
EXPECTED_CONTROL_REF = "exp/e3v13-transfer-telemetry-controls"
...
REGISTERED_CHANGES_FROM_E3V12 = {
    "reason": "synchronize_transferable_role_capacity_with_attempt_telemetry",
    ...
    "architect_unused_calls_transfer_to_developer_after_validated_handoff": True,
    "manager_final_review_call_reserved": True,
    "fixed_and_transferred_role_capacity_reported_separately": True,
    "telemetry_schema_version": {"from": "v3", "to": "v4"},
...
class ConfigError(ValueError):
    """The prospective E3V13 configuration violates a design constraint."""
...
class NotReadyError(RuntimeError):
    ...
    def __init__(self, issues: Iterable[str]):
        ...
        super().__init__("E3V13 is not ready: " + "; ".join(self.issues))
...
def validate_config(config: dict[str, Any]) -> None:
    ...
    if config["conditions"] != EXPECTED_CONDITIONS:
        raise ConfigError("M0 and M1 do not match the E3V13 architecture contrast")
    if config["arm_order"] != EXPECTED_ARM_ORDER:
        raise ConfigError("E3V13 must retain the alternating E3V8 pair order")
    public_paths = {
    ...
    if (
        ...
        raise ConfigError(
            "E3V13 must transfer only unused Architect planning calls after a "
            "validated handoff while reserving Manager's final review call"
        )
    if config["execution"]["formal_model_execution_enabled"]:
        raise ConfigError("the E3V13 preparation file must not enable model execution")
    if (
    ...
    if (
        config["freeze_status"] == "ready_for_freeze"
        and config["source"]["source_ref"] != "exp/shared-t3-runtime-v4"
    ):
        raise ConfigError("E3V13 must bind the published shared runtime v4 source ref")
    if (
        config["freeze_status"] == "ready_for_freeze"
        and config["control"]["control_ref"] != EXPECTED_CONTROL_REF
    ):
        raise ConfigError("E3V13 must bind the dedicated freeze-control ref")
...
def _run_id(replicate: int, arm: str) -> str:
    return f"E3V13-T3-R{replicate}-{arm}"
...
def build_run_identity(
    ...
    identity = {
        "schema_version": "exp3-e3v13-run-identity-draft-v1",
...
def validate_pair(pair: list[dict[str, Any]]) -> None:
    if len(pair) != 2:
        raise ConfigError("an E3V13 pair must contain exactly two observations")
