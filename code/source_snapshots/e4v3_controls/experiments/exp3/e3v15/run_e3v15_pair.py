#!/usr/bin/env python3
"""Preflight or serially execute one E3V15 T3 M0/M1 pair."""
...
try:
    from . import run_e3v15_observation as observation
    from .item_store_projection import project_item_store
except ImportError:  # pragma: no cover - direct script execution
    import run_e3v15_observation as observation  # type: ignore[no-redef]
...
def _fetch_submission(
    *, source_ref: str, source_commit: str, target_ref: str, destination: Path
    ...
    with tempfile.TemporaryDirectory(prefix="e3v15-score-git-") as raw:
...
def main() -> int:
    ...
    try:
        ...
        print(
            json.dumps(
                {
                    "schema_version": "exp3-e3v15-pair-result-v1",
                    "status": "failed",
