# E4V2 prospective protocol: T3 RAG × agent architecture

Status: prospective control awaiting immutable control-commit publication and
zero-model freeze.  No E4V2 model observation exists at this stage.
...
## Version boundary from E4V1

E4V1 remains immutable. E4V2 uses a new source commit, image, agreement, run
namespace and empty target branches. The new runtime discloses a 50,000
canonical-JSON-character limit before Architect and Developer write their
handoffs. It also distinguishes a non-array, an empty array and a nested array,
names the rejected calculation argument, and tells the model that it may retry
within the existing call cap. These changes do not add task hints, fill an
array, select an answer field, alter the two-attempt budget or change scoring.

E4V2 reuses the exact previously frozen T3 retrieval result. Retrieval depends
on the T3 query rather than the runtime source branch, so the freeze builder
requires the same repository, task answer-capture profile, model, query hash
and retrieval structure but does not require the historical retrieval request
to name the new implementation branch. The resulting context bytes and query
bytes are hash-bound again in the E4V2 agreement.
