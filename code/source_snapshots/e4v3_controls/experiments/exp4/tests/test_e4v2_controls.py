...
from experiments.exp4 import (
    ...
    build_e4v2_freeze_config as freeze_builder,
...
def _resolved_config() -> dict:
    ...
    replacements = {
        "__UNRESOLVED_SOURCE_REF__": "exp4/e4v2-shared-source",
        "__UNRESOLVED_SOURCE_COMMIT_SHA__": "a" * 40,
        "__UNRESOLVED_CONTROL_REF__": "exp4/e4v2-controls",
        ...
        "__UNRESOLVED_CORPUS_ID__": "e4v2-corpus",
        "__UNRESOLVED_INDEX_ID__": "e4v2-index",
        "__UNRESOLVED_EXCLUSION_LIST_ID__": "e4v2-exclusions",
    ...
    query_path = "experiments/exp4/E4V2_PROTOCOL.md"
...
def _public_inventory(config: dict) -> dict:
    t3 = config["task_bindings"]["T3"]
    contract = t3["result_contract"]
    public_files = {
        t3["task_text_path"]: t3["task_text_sha256"],
        t3["rubric_path"]: t3["rubric_sha256"],
        t3["item_submission_schema_path"]: t3["item_submission_schema_sha256"],
        t3["output_schema_path"]: t3["output_schema_sha256"],
        contract["scorer_result_schema_path"]: contract[
            "scorer_result_schema_sha256"
        ],
        contract["scorer_result_validator_path"]: contract[
            "scorer_result_validator_sha256"
        ],
        **{row["path"]: row["sha256"] for row in t3["public_inputs"]},
    }
    return {
        "schema_version": freeze_builder.PUBLIC_BINDING_SCHEMA,
        "status": "public_bindings_verified_private_bindings_unresolved",
        "provider_call_count": 0,
        "external_model_calls_made": False,
        "source": {
            "source_ref": freeze_builder.SOURCE_REF,
            "source_commit": "a" * 40,
        },
        "runtime": {
        ...
        "control": {"visible_response_trace_schema_sha256": "e" * 64},
    }


def _with_provider_identity(public: dict, config: dict) -> dict:
    public["runtime"]["provider_identity"] = {
        "schema_version": "llm-provider-identity-v1",
        "adapter": "openai_chat_completions",
        "provider_mode": config["runtime"]["provider_mode"],
        "base_url": config["runtime"]["provider_base_url"],
        "model_alias": config["runtime"]["model_alias"],
        "transport_model": config["runtime"]["model_alias"],
    }
    return public


def test_e4v2_resolves_v7_public_bindings_without_the_e3v17_v6_gate() -> None:
    expected = _resolved_config()
    config = identity.load_config()
    public = _with_provider_identity(_public_inventory(expected), expected)

    freeze_builder._copy_runtime_bindings(config, public)

    assert config["source"]["source_ref"] == "exp/shared-t3-runtime-v7"
    assert config["source"]["source_commit"] == "a" * 40
    assert config["runtime"]["image_digest"] == expected["runtime"]["image_digest"]
    assert config["task_bindings"]["T3"]["task_text_sha256"] == expected[
        "task_bindings"
    ]["T3"]["task_text_sha256"]


def test_e4v2_rejects_an_older_runtime_source_ref() -> None:
    expected = _resolved_config()
    public = _with_provider_identity(_public_inventory(expected), expected)
    public["source"]["source_ref"] = "exp/shared-t3-runtime-v6"

    with pytest.raises(
        freeze_builder.FreezeConfigError,
        match="must bind the published shared runtime v7 source ref",
    ):
        freeze_builder._copy_runtime_bindings(identity.load_config(), public)


def test_retrieval_context_can_be_rebound_from_the_same_task_on_an_older_source(
    tmp_path: Path,
) -> None:
    ...
    context["query_sha256"] = freeze_builder._sha256_bytes(
        context["query"].encode("utf-8")
    )
    request = {
        "repository_details": [
            {
                "repo_full_name": config["source"]["repository"],
                "source_branch": "historical-runtime-source",
            }
        ],
        "execution_objectives": {
            "parsed_task_parameters": {
                "rag_enabled": True,
                "model": config["runtime"]["model_alias"],
                "answer_capture_profile": config["task_bindings"]["T3"][
    ...
    freeze_builder._bind_retrieval(config, request, staging, output)

    binding = config["retrieval_binding"]["task_query_context"]["T3"]
    assert binding["retrieval_context_sha256"] == freeze_builder._sha256_bytes(
        freeze_builder._canonical(context)
    )
    assert (staging / "retrieval_context.json").is_file()
...
def test_current_builder_refuses_before_creating_output() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        ...
        with pytest.raises(identity.NotReadyError):
            build_e4v2_agreement.build_agreement(identity.load_config(), output)
        assert not output.exists()


def test_current_preflight_is_zero_call_and_unready() -> None:
    result = preflight_e4v2.preflight(
        identity.load_config(), condition_code="M1R1"
    )
...
def test_first_four_williams_rows_balance_and_fifth_is_registered_repeat() -> None:
    config = identity.load_config()
    ...
    assert config["williams_order"]["5"] == config["williams_order"]["1"]


def test_schedule_contains_five_four_observation_blocks() -> None:
    schedule = identity.build_schedule(_resolved_config())
    assert len(schedule) == 5
    assert len({block["block_id"] for block in schedule}) == 5
    assert sum(len(block["observations"]) for block in schedule) == 20
...
def test_block_projection_differs_only_in_treatment_and_logistics() -> None:
    block = identity.build_block_identities(_resolved_config(), "T3", 3)
...
def test_target_and_run_names_do_not_reveal_condition() -> None:
    block = identity.build_block_identities(_resolved_config(), "T3", 4)
    ...
    for item in block:
        ...
        assert item["target_ref"].startswith("quant/E4V2-T3-K4-P")


def test_resolved_preflight_still_refuses_without_reviewed_generation_gate() -> None:
    result = preflight_e4v2.preflight(
        _resolved_config(), condition_code="M1R1"
    )
    assert result["status"] == "unready"
    assert result["observation_count"] == 0
...
def test_resolved_builder_still_refuses_and_writes_nothing() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        ...
        with pytest.raises(identity.NotReadyError, match="does not enable"):
            build_e4v2_agreement.build_agreement(_resolved_config(), output)
        assert not output.exists()
...
def test_resolved_reviewed_config_can_enable_agreement_generation() -> None:
    config = _resolved_config()
...
def test_factorial_runtime_contract_and_two_attempt_budget_are_machine_fixed() -> None:
    ...
    assert budget == {
        ...
        "max_tokens_per_observation": 1000000,
...
def test_answer_capture_profile_is_explicit_for_t3() -> None:
    config = identity.load_config()
...
def test_t3_has_one_frozen_query_and_context_identity() -> None:
    config = identity.load_config()
    bindings = config["retrieval_binding"]["task_query_context"]

    assert set(bindings) == {"T3"}
...
def test_retrieval_context_declared_size_and_memory_count_are_verified() -> None:
    ...
    assert (
        ...
        "retrieval_binding.task_query_context.T3"
    ) in issues
    assert (
...
def test_m1r1_batch_contains_one_neutral_identity_per_replicate() -> None:
    batch = identity.build_condition_batch(_resolved_config(), "M1R1")
    assert len(batch) == 5
    assert [row["replicate"] for row in batch] == [1, 2, 3, 4, 5]
    assert all(row["architecture_mode"] == "manager_star" for row in batch)
    assert all(row["rag_enabled"] is True for row in batch)
    assert all(row["retrieval_context_included"] is True for row in batch)
    assert len({row["run_id"] for row in batch}) == 5
