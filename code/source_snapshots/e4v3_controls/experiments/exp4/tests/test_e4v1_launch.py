...
from experiments.exp4 import build_e4v1_agreement
from experiments.exp4 import build_e4v1_launch_package as launch_builder
from experiments.exp4 import identity, run_e4v1_batch, run_e4v1_observation
from experiments.exp4.tests.test_e4v1_controls import _resolved_config
...
def test_batch_records_one_launch_error_and_continues_remaining_runs(
    ...
    monkeypatch.setattr(run_e4v1_batch.observation, "execute", fake_execute)
    monkeypatch.setattr(
        run_e4v1_batch.shared_scoring,
        "_score",
        lambda state, launch: {
            "run_id": state["run_id"],
            "architecture_mode": "manager_star",
    ...
    monkeypatch.setattr(run_e4v1_batch, "RESULT_ROOT", tmp_path / "summaries")
    result = run_e4v1_batch.run_batch(Path("unused"), "M1R1", True)
...
def test_agreement_launch_package_and_observation_preflight_connect(
    ...
    agreement = build_e4v1_agreement.build_agreement(config, agreement_dir)
    ...
    monkeypatch.setattr(run_e4v1_observation, "_validate_control", lambda value: None)
    monkeypatch.setattr(run_e4v1_observation, "_image_available", lambda value: None)
    ...
    monkeypatch.setattr(run_e4v1_observation, "_remote_head", remote)
    selected = next(row for row in manifest["runs"] if row["condition_code"] == "M1R1")
    state = run_e4v1_observation.preflight(launch_dir, selected["run_id"])
