#!/usr/bin/env python3
"""Preflight or execute exactly one frozen E4 observation."""
...
def _validate_package(package: Path) -> tuple[dict[str, Any], dict[str, Any], list[dict[str, Any]]]:
    ...
    expected_schema = f"exp4-{str(experiment_id).lower()}-launch-package-v1"
    if experiment_id == "E4V2":
        profile_valid = manifest.get("observations_per_authorization") == 5
    else:
        registered = manifest.get("registered_conditions")
        registered_count = len(identity.CONDITION_CODES) * int(manifest.get("replicate_count", 0))
        profile_valid = (
            registered == list(identity.CONDITION_CODES)
            and manifest.get("replicate_count") == 15
            and manifest.get("registered_observation_count") == registered_count == 60
            and manifest.get("observations_per_authorization") is None
            and manifest.get("selection_mode") == "runtime_explicit_subset"
        )
    if (
        experiment_id not in {"E4V2", "E4V3"}
        ...
        raise ObservationLaunchError("launch package is not an approved frozen E4 profile")
...
def preflight(package: Path, run_id: str) -> dict[str, Any]:
    ...
    if config["experiment_id"] == "E4V3":
        history_root = PROJECT_ROOT / "02_EXPERIMENT_CONTROL/status-and-logs/e4v3-terra-observations"
        if history_root.is_dir() and any(history_root.glob(f"{run_id}-*")):
            raise ObservationLaunchError("formal run ID already has a local observation record")
    _image_available(config["runtime"]["image_digest"])
    return {
        "schema_version": f"exp4-{config['experiment_id'].lower()}-observation-preflight-v1", "status": "ready",
...
def execute(state: dict[str, Any], output_root: Path | None = None) -> dict[str, Any]:
    experiment_id = state["config"]["experiment_id"]
    authorization_name = f"{experiment_id}_RUN_AUTHORIZATION"
    if os.getenv(authorization_name) != f"AUTHORIZE_{state['run_id']}":
        raise ObservationLaunchError(f"set {authorization_name}=AUTHORIZE_{state['run_id']}")
    if output_root is None:
        output_root = DEFAULT_OUTPUT_ROOT if experiment_id == "E4V2" else PROJECT_ROOT / "02_EXPERIMENT_CONTROL/status-and-logs/e4v3-terra-observations"
    ...
    start = {
        "schema_version": f"exp4-{experiment_id.lower()}-formal-start-v1", "started_at": datetime.now(timezone.utc).isoformat(),
    ...
    launch_result = {
        "schema_version": f"exp4-{experiment_id.lower()}-local-launch-result-v1", "run_id": state["run_id"],
...
def main() -> int:
    ...
    try:
        ...
        print(json.dumps({"schema_version": "exp4-e4-observation-error-v1", "status": "failed", "provider_call_count": 0 if not args.execute else None, "formal_observation": bool(args.execute), "failure_type": type(exc).__name__, "message": str(exc)}, sort_keys=True))
        return 1
