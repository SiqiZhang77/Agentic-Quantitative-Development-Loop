# E4V1 T3 four-cell controls

E4V1 crosses the two switches already studied separately while keeping the T3
...
2. Run `build_e4v1_freeze_config.py` with the resolved E3V17 shared-runtime
   config, its public bindings and hash-only evaluator manifest, plus one
   verified E2V7 RAG-on request.  The script copies only the model-visible
   retrieval object; it never prints its text or reads answer keys.
3. Run `build_e4v1_agreement.py`.  It freezes all 20 identities and the declared
   phase order without making model calls.
4. Run `build_e4v1_launch_package.py`.  It builds 20 request/negative-ref
   records and binds the clean control commit, still with zero model calls.
5. Run `run_e4v1_batch.py --condition M1R1` without `--execute` as the final
