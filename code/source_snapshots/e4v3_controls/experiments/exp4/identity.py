...
EXPECTED_CONTROL_FILES = (
    ...
    "experiments/exp4/build_e4v1_freeze_config.py",
    "experiments/exp4/build_e4v1_agreement.py",
    "experiments/exp4/preflight_e4v1.py",
    "experiments/exp4/build_e4v1_launch_package.py",
    "experiments/exp4/run_e4v1_observation.py",
    "experiments/exp4/run_e4v1_batch.py",
)
...
_NEUTRAL_TARGET = re.compile(r"^quant/E4V1-T3-K[1-5]-P[1-4]$")
...
def _validate_williams_order(config: dict[str, Any]) -> None:
    ...
    rows = [tuple(orders[str(replicate)]) for replicate in range(1, 6)]
    ...
    balanced_rows = rows[:4]
    for position in range(4):
        if {row[position] for row in balanced_rows} != expected:
            raise ConfigError("the first four rows must place each condition in every position")
    transitions = [
        (left, right)
    ...
    if len(transitions) != 12 or set(transitions) != expected_transitions:
        raise ConfigError("the first four Williams rows must balance all 12 transitions")
    if rows[4] != rows[0]:
        raise ConfigError("the pre-registered fifth row must repeat the first Williams row")
...
def build_run_identity(
    ...
    *,
    ...
    if replicate not in range(1, config["replicate_count"] + 1):
        raise ConfigError("replicate is outside 1..5")
...
def build_schedule(config: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    if len(records) != 5 or sum(len(item["observations"]) for item in records) != 20:
        raise AssertionError("E4V1 schedule must contain 5 blocks and 20 observations")
    return records
