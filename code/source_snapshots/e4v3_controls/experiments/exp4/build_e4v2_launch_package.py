#!/usr/bin/env python3
"""Build a runnable, zero-model E4 four-cell launch package."""
...
try:
    from . import identity_e4v2 as identity, preflight_e4v2
except ImportError:  # pragma: no cover
    import identity_e4v2 as identity  # type: ignore[no-redef]
    import preflight_e4v2  # type: ignore[no-redef]
...
PRIOR_STUDY_REFS = (
    ...
    *(f"quant/E4V{version}-T3-K{replicate}-P{position}" for version in (1, 2) for replicate in range(1, 6) for position in range(1, 5)),
)
...
def _flatten_schedule(schedule: list[dict[str, Any]], expected_count: int) -> list[dict[str, Any]]:
    values = [copy.deepcopy(item) for block in schedule for item in block.get("observations", [])]
    if len(values) != expected_count or len({row.get("run_id") for row in values}) != expected_count:
        raise LaunchPackageError("agreement schedule does not contain the frozen number of unique observations")
    return values
...
def _request(
    *,
    ...
    launch_text = f"Execute the complete frozen {experiment_id} T3 task using only the declared source, target and shared tools."
    value: dict[str, Any] = {
        "schema_version": "1.0",
        "run_id": run_id,
        ...
        "jira_metadata": {
            ...
            "summary": f"{experiment_id} T3 replicate {run_identity['replicate']}",
            "description": f"Complete the hash-bound {experiment_id} T3 v2 task supplied as the canonical parsed objective.",
...
def _negative_manifest(run_identity: dict[str, Any], all_runs: list[dict[str, Any]]) -> tuple[dict[str, Any], str, str]:
    ...
    prior_refs = [
        ref for ref in PRIOR_STUDY_REFS
        if ref not in {source_ref, current, paired, *other_results}
    ]
    experiment_id = run_identity["experiment_id"]
    probe_a = f"exp4/probe/{experiment_id}/{run_identity['run_id']}/a"
    probe_b = f"exp4/probe/{experiment_id}/{run_identity['run_id']}/b"
    value = {
...
def build(
    *,
    ...
    selected_conditions = identity.selected_condition_codes(config)
    expected_observation_count = len(selected_conditions) * replicate_count
    for condition in selected_conditions:
        result = preflight_e4v2.preflight(config, condition_code=condition)
        if result["status"] != "ready" or result["provider_call_count"] != 0 or result["observation_count"] != replicate_count:
    ...
    identities = _flatten_schedule(schedule, expected_count=expected_observation_count)
    ...
    try:
        ...
        manifest: dict[str, Any] = {
            "schema_version": f"exp4-{config['experiment_id'].lower()}-launch-package-v1",
            "status": "execution_authorized_not_started", "experiment_id": config["experiment_id"],
            ...
            "serial": True, "replicate_count": replicate_count,
            "observations_per_authorization": config["execution"]["observations_per_authorization"],
            "continue_after_observation_failure": True,
            "first_authorized_condition": config["execution"]["first_authorized_condition"],
            "launch_control": launch_control, "evidence_sha256": evidence_files,
            "runs": run_records,
        }
        if config["experiment_id"] == "E4V3":
            manifest.update({
                "registered_conditions": list(selected_conditions),
                "registered_observation_count": expected_observation_count,
        ...
        _write_json(staging / "launch_manifest.sha256.json", {"schema_version": f"exp4-{config['experiment_id'].lower()}-launch-package-sha256-v1", "launch_package_sha256": manifest["launch_package_sha256"]})
