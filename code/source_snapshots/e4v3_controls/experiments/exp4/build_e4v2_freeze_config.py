#!/usr/bin/env python3
"""Resolve an E4 four-cell study from reviewed shared-runtime bindings and RAG context.
...
REMOTE_NAME = "github"
SOURCE_REF = "exp/shared-t3-runtime-v7"
PUBLIC_BINDING_SCHEMA = "shared-t3-e3v17-public-binding-inventory-v1"
PRIVATE_EVALUATOR_SCHEMA = "exp3-e3v17-private-evaluator-hashes-v1"
ITEM_STORE_PROJECTION = (
    CONTROL_ROOT / "experiments" / "exp3" / "e3v17" / "item_store_projection.py"
)
...
def _require_sha256(value: Any, label: str) -> str:
    if (
        ...
        raise FreezeConfigError(f"{label} is not one lowercase SHA-256 value")
    return value


def _validate_public(public: dict[str, Any]) -> None:
    if public.get("schema_version") != PUBLIC_BINDING_SCHEMA:
        raise FreezeConfigError("public binding inventory schema is not reviewed")
    if public.get("status") != "public_bindings_verified_private_bindings_unresolved":
        raise FreezeConfigError("public binding inventory did not finish successfully")
    if (
        public.get("provider_call_count") != 0
        or public.get("external_model_calls_made") is not False
    ):
        raise FreezeConfigError("public binding inventory does not prove zero model calls")
    source = public.get("source") or {}
    if source.get("source_ref") != SOURCE_REF:
        raise FreezeConfigError("E4 must bind the published shared runtime v7 source ref")
    ...
    if (
        ...
        raise FreezeConfigError("public source commit is not one lowercase Git SHA")


def _validate_private(evaluator: dict[str, Any]) -> None:
    expected = {
        "schema_version",
        "task_suite_version",
    ...
    if set(evaluator) != expected:
        raise FreezeConfigError("private evaluator hash manifest fields differ")
    if evaluator["schema_version"] != PRIVATE_EVALUATOR_SCHEMA:
        raise FreezeConfigError("private evaluator hash manifest schema is not reviewed")
    if evaluator["task_suite_version"] != "t3-quant-suite-v2":
        raise FreezeConfigError("private evaluator is not bound to T3 suite v2")
    for field in (
        "private_item_scorer_sha256",
        "private_base_scorer_sha256",
        "evaluator_coordinator_sha256",
    ):
        _require_sha256(evaluator[field], f"private evaluator {field}")
    if (
        evaluator["provider_call_count"] != 0
        or evaluator["external_model_calls_made"] is not False
    ):
    ...
    for field in (
        ...
        if evaluator[field] is not False:
...
def _public_file(public: dict[str, Any], relative: str) -> str:
    ...
    return _require_sha256(files[relative], f"public file {relative}")


def _copy_runtime_bindings(config: dict[str, Any], public: dict[str, Any]) -> None:
    _validate_public(public)
    source = public["source"]
    runtime_binding = public.get("runtime") or {}
    provider_identity = runtime_binding.get("provider_identity") or {}
    if provider_identity.get("schema_version") != "llm-provider-identity-v1":
        raise FreezeConfigError("public provider identity schema is not reviewed")
    if provider_identity.get("adapter") != "openai_chat_completions":
        raise FreezeConfigError("public provider adapter differs from the reviewed E4 profile")
    config["source"] = {
    ...
    config["runtime"]["provider_mode"] = provider_identity.get("provider_mode")
    config["runtime"]["provider_base_url"] = provider_identity.get("base_url")
    config["runtime"]["model_alias"] = provider_identity.get("model_alias")

    t3 = config["task_bindings"]["T3"]
    mapping = {
        "task_text_sha256": t3["task_text_path"],
        "rubric_sha256": t3["rubric_path"],
        "item_submission_schema_sha256": t3["item_submission_schema_path"],
        "output_schema_sha256": t3["output_schema_path"],
    }
    for target, relative in mapping.items():
        t3[target] = _public_file(public, relative)
    for row in t3["public_inputs"]:
        row["sha256"] = _public_file(public, row["path"])
    contract = t3["result_contract"]
    contract["scorer_result_schema_sha256"] = _public_file(
        public, contract["scorer_result_schema_path"]
    )
    contract["scorer_result_validator_sha256"] = _public_file(
        public, contract["scorer_result_validator_path"]
    )
...
def _copy_evaluator_bindings(
    ...
    public_control = public.get("control") or {}
    target["visible_response_trace_schema_sha256"] = _require_sha256(
        public_control.get("visible_response_trace_schema_sha256"),
        "visible response trace schema",
    )
    target["item_store_projection_sha256"] = _sha256_file(ITEM_STORE_PROJECTION)
...
def _bind_retrieval(
    ...
    if (
        len(repositories) != 1
        or repositories[0].get("repo_full_name") != config["source"]["repository"]
    ...
    if retrieval["requested_top_k"] != 5 or retrieval[
        ...
        raise FreezeConfigError("retrieval size policy differs from the reviewed E4 profile")
...
def build(
    *,
    ...
    _copy_runtime_bindings(config, public)
    ...
    try:
        ...
        receipt = {
            "schema_version": f"exp4-{config['experiment_id'].lower()}-freeze-receipt-v1",
...
def main() -> int:
    ...
    try:
        ...
        print(json.dumps({
            "schema_version": "exp4-e4-freeze-status-v1",
            "status": "blocked",
            "provider_call_count": 0,
            "formal_observation_count": 0,
            "issue": str(exc),
