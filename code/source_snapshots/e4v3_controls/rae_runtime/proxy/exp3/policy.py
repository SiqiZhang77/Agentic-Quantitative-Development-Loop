...
# These tools are exposed only by the request-bound calculator profile. The
# checkpoint writes only to the run evidence directory; the commit tool is the
# only calculator-profile tool that writes to the task repository.
CALCULATION_TOOLS: Final = frozenset(
    {
        "quant_calculate",
        "submit_t3_items",
...
ALL_TOOLS: Final = READ_TOOLS | DEVELOPER_WRITE_TOOLS | CALCULATION_TOOLS
