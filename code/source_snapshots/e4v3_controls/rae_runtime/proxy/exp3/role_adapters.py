...
def build_mcp_launch_spec(
    ...
    *,
    ...
    # The calculator is a request-bound tool profile, never an ambient child
    # environment capability inherited from the host.
    env.pop("RAE_ENABLE_QUANT_CALCULATOR", None)
    env.pop("MAX_QUANT_CALCULATE_CALLS", None)
    env.pop("WRITABLE_PATHS_MAP", None)
    env.pop("CALCULATION_SCHEMA_PATHS_MAP", None)
    env.pop("RAE_EXPERIMENT_ATTEMPT", None)
    ...
    calculator_enabled = quant_calculator_enabled(payload)
    allowed_tools = set(allowed_tools_for_role(known_role))
    if known_role in {MANAGER, ARCHITECT}:
        env["MAX_BRANCHES_PER_RUN"] = "0"
        env["MAX_COMMITS_PER_RUN"] = "0"
        allowed_tools.difference_update(CALCULATION_TOOLS)
        ...
        if calculator_enabled:
            env["RAE_ENABLE_QUANT_CALCULATOR"] = "true"
            env["MAX_QUANT_CALCULATE_CALLS"] = str(MAX_CALLS_PER_RUN)
            current_attempt = attempt_number(payload)
            if current_attempt is not None:
                env["RAE_EXPERIMENT_ATTEMPT"] = str(current_attempt)
            env["WRITABLE_PATHS_MAP"] = json.dumps(
                quant_calculator_writable_paths(payload),
                sort_keys=True,
            )
            env["CALCULATION_SCHEMA_PATHS_MAP"] = json.dumps(
