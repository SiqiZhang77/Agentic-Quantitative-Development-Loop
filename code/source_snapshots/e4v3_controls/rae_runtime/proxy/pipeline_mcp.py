...
_WRITE_TOOLS = frozenset(
    {"commit_and_push", "replace_in_file", "commit_calculation_artifact"}
)
_MCP_FORBIDDEN_CREDENTIALS = frozenset(
    {
        "API_KEY",
        "OPENAI_API_KEY",
        "LITELLM_API_KEY",
        ...
        self.pipeline_out = {
            ...
            "diagnostics": {
                "retry_safe": {
                    ...
                    "tool_calls": audit,
                    "validation_reached": any(
                        item.get("tool")
                        in {"validate_content", "commit_calculation_artifact"}
                        for item in audit
                    ),
...
async def _close_owned_agents_model_client(model: object) -> None:
    """Await the direct OpenAI client before asyncio closes this run's loop."""

    client = getattr(model, "_rae_owned_async_client", None)
    if client is None:
        return
    close = getattr(client, "close", None) or getattr(client, "aclose", None)
    if not callable(close):
        raise RuntimeError("owned OpenAI client does not expose async cleanup")
    result = close()
    if hasattr(result, "__await__"):
        await result


def _mcp_child_environment(payload: dict) -> dict[str, str]:
    """Return the inherited MCP environment with provider secrets removed.

    Calculator exposure is derived only from the structured request flag. Host
    environment flags cannot widen the model-facing tool surface.
    """

    environment = os.environ.copy()
    for secret_name in _MCP_FORBIDDEN_CREDENTIALS:
        environment.pop(secret_name, None)
    environment.pop("RAE_ENABLE_QUANT_CALCULATOR", None)
    environment.pop("MAX_QUANT_CALCULATE_CALLS", None)
    environment.pop("WRITABLE_PATHS_MAP", None)
    environment.pop("CALCULATION_SCHEMA_PATHS_MAP", None)
    environment.pop("RAE_EXPERIMENT_ATTEMPT", None)
    if quant_calculator_enabled(payload):
        environment["RAE_ENABLE_QUANT_CALCULATOR"] = "true"
        environment["MAX_QUANT_CALCULATE_CALLS"] = str(MAX_CALLS_PER_RUN)
        current_attempt = attempt_number(payload)
        if current_attempt is not None:
            environment["RAE_EXPERIMENT_ATTEMPT"] = str(current_attempt)
        environment["WRITABLE_PATHS_MAP"] = json.dumps(
            quant_calculator_writable_paths(payload),
            sort_keys=True,
        )
        environment["CALCULATION_SCHEMA_PATHS_MAP"] = json.dumps(
...
def _mcp_tool_audit_path(payload: dict) -> Path:
    artifact_dir = Path(
        (payload.get("output_paths") or {}).get("artifact_dir")
        or "/workspace/output/artifacts"
    )
    try:
        artifact_dir.mkdir(parents=True, exist_ok=True)
    except OSError:
        artifact_dir = Path("/tmp")
    name = "mcp_tool_audit.jsonl"
    if "architecture_mode" in payload:
        from exp3.attempt_control import audit_filename

        name = audit_filename(payload, name)
...
def _capture_single_agent_visible_output(
    audit_path: Path,
    payload: dict,
    output,
) -> dict:
    """Record M0's visible answer without making the recorder part of the result."""

    return capture_model_visible_output(
        audit_path,
        architecture_mode="single_agent",
        role="developer",
        phase="single_agent_developer",
        attempt=attempt_number(payload),
        output=output,
    )
...
def _extract_commit_diagnostics(result, branch_name: str) -> dict:
    ...
    for item in getattr(result, "new_items", None) or []:
        ...
        requested_paths = (
            list(output.get("paths"))
            if isinstance(output.get("paths"), list) and output.get("paths")
            else []
        )
        successful_paths = []
        if tool_name == "read_files" and isinstance(output.get("files"), list):
            successful_paths = [
                file_result.get("path")
                for file_result in output["files"]
                if isinstance(file_result, dict)
                and file_result.get("status") == "success"
                and file_result.get("path") in requested_paths
            ]
        tool_calls.append(
            {
                "tool": tool_name,
                ...
                **({"paths": requested_paths} if requested_paths else {}),
                **(
    ...
    if source_path and source_path != target_path:
        ...
        read_declared_source = any(
            (
                ...
                and call.get("tool") == "read_file"
                and call.get("path") == source_path
            )
            or (
                call.get("tool") == "read_files"
                and (
                    source_path in (call.get("successful_paths") or [])
                    or (
                        call.get("status") == "success"
...
async def _run_pipeline_mcp_async(
    ...
    # The model runs in the parent process. The MCP child needs the repository
    # credential only; provider/AWS credentials are removed before spawning it.
    server_env = _mcp_child_environment(payload)
    ...
    audit_path = _mcp_tool_audit_path(payload)
    try:
    ...
    async with MCPServerStdio(
        ...
        try:
            ...
            try:
                await _close_owned_agents_model_client(raw_model)
            except Exception:
                # Preserve the model/runner failure when one already exists.  If
                # the run itself succeeded, a client that cannot close is a real
                # runtime failure rather than a harmless background warning.
                if not runner_failed:
                    raise

    sdk_usage = result.context_wrapper.usage
    _capture_single_agent_visible_output(audit_path, payload, result.final_output)
...
def run_pipeline_mcp(payload: dict, tracer=None, exp3_provider_boundary=None) -> dict:
    ...
    try:
        ...
        if quant_calculator_enabled(payload):
            try:
                exc.calculation_tool_calls = read_quant_calculate_records(
                    _mcp_tool_audit_path(payload)
                )
            except Exception:
                exc.calculation_tool_calls = []
        if exp3_provider_boundary is not None:
            accounting = exp3_provider_boundary.snapshot()
            accounting["calculation_tool_calls"] = getattr(
                exc, "calculation_tool_calls", []
            )
    ...
    if quant_calculator_enabled(payload):
        calculation_tool_calls = read_quant_calculate_records(
            _mcp_tool_audit_path(payload)
        )
        result["calculation_tool_calls"] = calculation_tool_calls
    if exp3_provider_boundary is not None:
        ...
        accounting["calculation_tool_calls"] = calculation_tool_calls
