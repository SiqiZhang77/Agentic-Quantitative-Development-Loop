...
EXISTING_TOOLS = READ_TOOLS | WRITE_TOOLS
ALL_TOOLS = EXISTING_TOOLS | CALCULATION_TOOLS
...
def _visible_tools() -> set[str]:
    ...
    visible_candidates = set(EXISTING_TOOLS)
    if server.os.getenv("RAE_ENABLE_QUANT_CALCULATOR") == "true":
        visible_candidates.update(CALCULATION_TOOLS)
    for tool_name in sorted(visible_candidates):
...
def test_developer_mcp_catalogue_exposes_scoped_read_and_write_tools(
    ...
    assert _visible_tools() == EXISTING_TOOLS


def test_enabled_developer_catalogue_adds_quant_calculate(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setenv("RAE_ARCHITECTURE_MODE", "manager_star")
    monkeypatch.setenv("RAE_AGENT_ROLE", "developer")
...
@pytest.mark.parametrize("role", ["manager", "architect"])
def test_enabled_calculator_remains_hidden_from_non_developer_roles(
    monkeypatch: pytest.MonkeyPatch,
    role: str,
) -> None:
    monkeypatch.setenv("RAE_ARCHITECTURE_MODE", "manager_star")
...
def test_completed_tool_result_survives_transcript_capture_failure(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setenv("RAE_ARCHITECTURE_MODE", "single_agent")
    calls: list[dict[str, object]] = []
    ...
    result = _wrapped_backend("submit_t3_items", calls)(
        request={"schema_version": "t3-quant-item-submission-v2", "items": []}
    )
    ...
    audit_path = Path(server.os.environ["MCP_TOOL_AUDIT_PATH"])
    record = json.loads(audit_path.read_text(encoding="utf-8"))
    assert record["status"] == "success"
    assert record["transcript_capture_status"] == "failed"
    assert record["transcript_capture_error"] == (
        "model_visible_transcript_write_failed"
    )
    assert record["transcript_capture_evidence_status"] == "persisted"
