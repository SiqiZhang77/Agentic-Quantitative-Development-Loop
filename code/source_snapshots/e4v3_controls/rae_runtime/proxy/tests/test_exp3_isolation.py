...
def _formal_manifest_value() -> dict:
    return {
        "schema_version": "rae-negative-ref-manifest-v2",
        "experiment_id": "E2-terra-t3-rag-v3",
        "run_id": "E2T3S-ABC123",
        "repositories": [
            {
                "repo_full_name": "bankingscience/BSLAgenticQuantDevLoop",
                "source_ref": "exp2/terra-t3-rag-source-v3",
                "current_target_ref": "quant/E2T3RAGV3-A",
                "paired_target_ref": "quant/E2T3RAGV3-B",
                "deny_refs": {
                    "prior_study_refs": ["exp2/terra-t3-rag-source-v2"],
                    "other_result_refs": ["quant/E2T3RAGV3-C"],
                    "protected_refs": ["main"],
                    "arbitrary_probe_refs": [
                        "exp2/probe/E2T3RAGV3/a",
                        "exp2/probe/E2T3RAGV3/b",
                    ],
                },
            }
        ],
...
def test_neutral_formal_manifest_is_versioned_and_preserves_two_ref_allowlist() -> None:
    value = _formal_manifest_value()
    manifest = NegativeRefManifest.from_dict(value)

    assert manifest.schema_version == "rae-negative-ref-manifest-v2"
    assert manifest.experiment_id == "E2-terra-t3-rag-v3"
    policy = manifest.repositories[0]
    assert policy.readable_refs == {
        "exp2/terra-t3-rag-source-v3",
        "quant/E2T3RAGV3-A",
    }
    assert "main" in policy.complete_deny_set
    assert "quant/E2T3RAGV3-B" in policy.complete_deny_set
    assert not policy.can_read("quant/E2T3RAGV3-C")


def test_neutral_formal_manifest_rejects_cross_category_duplicates() -> None:
    value = _formal_manifest_value()
    value["repositories"][0]["deny_refs"]["other_result_refs"].append(
        "exp2/terra-t3-rag-source-v2"
    )

    with pytest.raises(IsolationManifestError, match="globally unique"):
...
def test_continuation_preflight_keeps_isolation_but_does_not_require_target_absent(
) -> None:
    manifest = NegativeRefManifest.from_dict(_manifest_value())
    source = "exp3/frozen-source"
    target = "quant/E3-T1-R1-M1"
    existing = {source, target}
    preflight = NegativeRefPreflight(
        manifest,
        ref_exists=lambda _repo, ref: ref in existing,
        require_current_target_absent=False,
    )

    evidence = preflight.pre_orchestration()
    before_call = preflight.before_provider_call(
        role="developer",
    ...
    assert before_call["passed"] is True
