...
def _vector(value: Any, *, booleans: bool = False) -> list[Any]:
    if not isinstance(value, list) or not value or any(isinstance(item, list) for item in value):
        raise QuantCalculationError("nonempty_vector_required")
    if len(value) > MAX_VECTOR_LENGTH:
        raise QuantCalculationError("vector_too_large")
    if booleans:
        if any(type(item) is not bool for item in value):
            raise QuantCalculationError("boolean_mask_required")
        return list(value)
...
def _matrix(value: Any) -> list[list[float]]:
    if not isinstance(value, list) or not value:
        raise QuantCalculationError("nonempty_matrix_required")
    if len(value) > MAX_VECTOR_LENGTH:
        raise QuantCalculationError("matrix_too_large")
    rows = [_vector(row) for row in value]
    width = len(rows[0])
    if width > MAX_MATRIX_COLUMNS or any(len(row) != width for row in rows):
        raise QuantCalculationError("invalid_matrix_shape")
    return rows
...
def _rebalance_path(args: Mapping[str, Any]) -> list[list[float]]:
    ...
    returns = _matrix(args["returns"])
    target = _vector(args["target_weights"])
...
def _execute_operation(operation: str, args: Mapping[str, Any]) -> Any:
    ...
    if operation in {"covariance", "correlation"}:
        _require_operation_args(operation, args)
        left = _vector(args["left"])
        right = _vector(args["right"])
    ...
    if operation in {"greater_than", "less_than"}:
        ...
        if isinstance(left, list) and type(right) in (int, float):
            return [comparator(value, _number(right)) for value in _vector(left)]
        if type(left) in (int, float) and isinstance(right, list):
            return [comparator(_number(left), value) for value in _vector(right)]
        if isinstance(left, list) and isinstance(right, list):
    ...
    if operation == "filter":
        _require_operation_args(operation, args)
        values = _vector(args["values"])
        mask = _vector(args["mask"], booleans=True)
