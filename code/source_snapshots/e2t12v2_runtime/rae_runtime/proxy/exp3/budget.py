...
SHARED_MAX_CALLS: Final = 15
SHARED_MAX_TOKENS: Final = 200_000
ROLE_LIMITS: Final = MappingProxyType(
    {
        MANAGER: MappingProxyType({"max_calls": 3, "max_tokens": 45_000}),
        ARCHITECT: MappingProxyType({"max_calls": 3, "max_tokens": 35_000}),
        DEVELOPER: MappingProxyType({"max_calls": 9, "max_tokens": 120_000}),
    }
)
...
class RoleBudgetLedger:
    ...
    Role capacity is never pooled: both the role limit and the shared limit are
    independently checked.  The sum of the three role buckets is the shared
    snapshot, avoiding a second mutable total that could drift under concurrency.
    """
    ...
    def for_single_agent(cls) -> "RoleBudgetLedger":
        ...
        Experiment 3 assigns role partitions only to M1.  M0 therefore receives
        15 calls and 200,000 tokens without inheriting M1's 9/120,000 developer
        bucket.  Manager and architect have zero capacity in this ledger.
        """
